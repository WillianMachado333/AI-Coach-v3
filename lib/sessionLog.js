...
function isTester({ email, userId, objectId } = {}) {
...
function startSession({ email, userId, objectId, caller, url } = {}) {
    ...
    const meta = {
        ...
        actor: {
            ...
            caller: caller || null,
            url: url || null,
            tester: isTester({ email, userId, objectId })
        },
        env: {
            store: STORE_MODE
        }
...
function _redactUserText(text) {
    if (STORE_MODE === 'raw') return text;
    if (!text) return null;
    const t = String(text);
    return {
        redacted: true,
...
/**
 * Log a user turn. Handles the STORE_MESSAGE_TEXT redaction switch.
 */
