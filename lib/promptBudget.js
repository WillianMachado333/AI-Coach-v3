...
function compute({ personaName = null } = {}) {
    ...
    if (!personaRead && chosen) {
        ...
        // Framework file names on disk are prefixed with `framework_`
        // (framework_Supportive.md, framework_Directive.md, etc.), so aliases
        // point at the prefixed forms.
        const labelAlias = {
            'calm, reassuring coach': 'framework_Supportive',
            'reassuring coach': 'framework_Supportive',
            'supportive coach': 'framework_Supportive',
            'directive coach': 'framework_Directive',
            'discovery coach': 'framework_Discovery',
            'nurturing coach': 'framework_Nurturing',
            'exploratory coach': 'framework_Exploratory',
            'guidance coach': 'framework_Guidance',
            'empowering coach': 'framework_Empowering',
            'strengths coach': 'framework_Strengths'
        };
        const aliased = labelAlias[key];
        if (aliased && frameworks.includes(aliased)) {
            personaRead = contentStore.readFramework(aliased);
        }
        // Last resort: any framework name that appears as a substring (also
        // strip the framework_ prefix from disk names so a plain "Supportive"
        // still matches a file named framework_Supportive).
        if (!personaRead) {
            const fuzzy = frameworks.find((n) => {
                const bare = n.replace(/^framework_/, '').toLowerCase();
                return key.includes(bare);
            });
    ...
    blocks.push({
        ...
        note: personaRead
            ? 'Persona intro + tone + full behavioural guardrails. Sits at the TOP of the prompt (primacy). Sourced from ' + (personaRead.source === 'overlay' ? 'the edited overlay' : 'the repo default') + ' (' + personaRead.name + '.md).'
    ...
    blocks.push({
        ...
        note: 'Short summary of the persona rules appended right after the persona intro. Same guardrails as the persona file, expressed as tight directives.'
    });
    blocks.push({
        key: 'wixPreamble',
        name: 'External prep from Wix (rules + user report)',
        chars: (last && last.blocks && last.blocks.wixPreamble) || 17000,
        source: (last && last.blocks && last.blocks.wixPreamble) ? 'reported' : 'estimated',
        note: 'Monolithic block coming from Wix on preparation: global rules, resource lists (URLs), and the current user report body. Migration goal: split this into Injected Data (rules + canonical lists) + user report as its own runtime channel.'
    });
    blocks.push({
        key: 'knowledgeGrounding',
        name: 'Coach tool + visual widget rules',
        ...
        note: 'How the coach uses her tools: when to search_knowledge, when to deep_think, when to render_chart or render_table. Also the element-focus context rules. These are the "how" — the content itself lives in the Semantic Store.'
    });
    blocks.push({
        key: 'activity',
        ...
        note: channelState.activity_timeline
            ? 'Recent user events from CleverTap (took quiz, read report, visited a page), injected on session boot so Erica can reference them.'
            : 'Channel currently OFF in Real Time. Nothing injected.'
    });
    blocks.push({
        key: 'pageContext',
        ...
        note: channelState.page_context
            ? 'Snapshot of the current parent page (title, headings, main text) so Erica can respond to what the user is looking at.'
            : 'Channel currently OFF in Real Time.'
    });
    blocks.push({
        ...
        source: 'measured',
        note: 'Fixed instruction to always answer in the language of the most recent user message. Small but critical for multilingual sessions.'
    });
    blocks.push({
        key: 'reinforcement',
        ...
        source: 'measured',
        note: 'Very last thing in the prompt — a 200-char summary of the persona rules. LLMs weight the LAST tokens heavily (recency), so this short reminder is critical even though it looks tiny next to the 2k persona block at the top.'
    });
