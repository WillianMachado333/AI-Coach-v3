...
function compute({ personaName = null } = {}) {
    ...
    const personaRead = chosen ? contentStore.readFramework(chosen) : null;
    ...
    blocks.push({
        ...
        chars: personaChars,
        source: personaRead ? 'measured' : 'missing',
        note: personaRead
            ? 'From ' + (personaRead.source === 'overlay' ? 'edited overlay' : 'repo default')
