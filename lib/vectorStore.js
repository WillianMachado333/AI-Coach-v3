...
/** Return the OpenAI client if initialised (or null). Used by studioAgent. */
...
async function syncUserReport(userId, reportText) {
    ...
    try {
        ...
        for (const f of existing.data || []) {
            ...
            try {
                // OpenAI SDK v4 uses `del` (not `delete` — reserved JS keyword)
                // for vector store file removal. Signature: del(vectorStoreId, fileId).
                await client.vectorStores.files.del(storeId, f.id);
                try { await client.files.del(f.id); } catch (_) { /* orphan cleanup best-effort */ }
                purgedCount++;
 ...
 */
async function searchKnowledge({ query, scope = 'all', userId = null, objectId = null, model, maxResults = 5 }) {
    if (!query || !query.trim()) {
    ...
    const wantCourses = scope === 'courses' || scope === 'all';
    ...
    if (wantCourses && process.env.COURSES_STORE_ID) {
        vectorStoreIds.push(process.env.COURSES_STORE_ID);
    }
    if (wantUserData) {
        // Signed-in user store keyed by userId, guest store keyed by
        // "guest-<objectId>" (same convention used by syncUserActivity).
        if (userId) {
            const uid = await getUserVectorStoreId(userId);
            if (uid) vectorStoreIds.push(uid);
        } else if (objectId) {
            const gid = await getUserVectorStoreId('guest-' + objectId);
            if (gid) vectorStoreIds.push(gid);
        }
    }
...
async function deepThink({ query, context = '', model } = {}) {
    ...
    // summary:'auto' asks the reasoning model to emit a compact rationale
    // trace alongside the answer. We capture it via response.output items
    // of type 'reasoning' (see below) and expose it so admin/session log
    // can render it. BUT: some orgs cannot request summaries until they
    // complete OpenAI's organization verification; the API returns 400 in
    // that case. Detect the specific verification error and retry without
    // summary so the coach never breaks on this feature.
    let response;
    let summaryDisabledReason = null;
    try {
        ...
        if (/verified.*generate reasoning summaries/i.test(msg) || /Verify Organization/i.test(msg)) {
            console.warn('[vectorStore.deepThink] reasoning.summary disabled — org not verified. Falling back without summary. Message:', msg.slice(0, 200));
            summaryDisabledReason = 'org_not_verified';
            response = await client.responses.create({
                model: reasoningModel,
                input: [
                    { role: 'system', content: systemPrompt },
    ...
    return {
        ...
        // Raw reasoning summary from the model's built-in reasoning trace
        // (populated when summary:'auto' returns a rationale). Kept separate
        // from `reasoning` (which is the model's structured JSON reasoning)
        // so callers can decide which to expose.
        reasoningSummary: reasoningTrace || null,
...
/**
 * Generate 3 short user-voice follow-up questions given the recent
 * conversation. Used to power the dynamic "quick-action" pills the user
 * sees after each coach reply. Small/fast model (gpt-4.1-mini) — we pay
 * this on every bot final so latency + cost matter.
 *
 * @param {object} opts
 * @param {Array<{role: string, content: string}>} opts.messages
 *        Last ~8 messages in order (oldest first). role in ('user','coach','system').
 * @param {string} [opts.persona] Optional persona label (Supportive, Directive, ...) to nudge tone.
 * @returns {Promise<{suggestions: string[]}>}
 */
async function suggestFollowUps({ messages = [], persona = '', activity = null, model } = {}) {
    const filtered = (Array.isArray(messages) ? messages : [])
        .filter((m) => m && typeof m.content === 'string' && m.content.trim())
        .slice(-8);

    // Need either conversation history OR user activity to produce
    // meaningful suggestions; otherwise return empty and let the client
    // fall back to static persona defaults.
    const hasActivity = activity && typeof activity === 'string' && activity.trim();
    if (filtered.length === 0 && !hasActivity) {
        return { suggestions: [] };
    }

    const client = requireClient();
    const suggestionModel = model || process.env.SUGGESTIONS_MODEL || 'gpt-4.1-mini';

    const transcript = filtered
        .map((m) => `${m.role === 'user' ? 'User' : 'Coach'}: ${m.content}`)
        .join('\n');

    // For starter suggestions (no conversation yet), tailor the framing so
    // the model produces OPENERS grounded in the activity. For continuation
    // it should still lean on the latest coach turn.
    const isStarter = filtered.length === 0;

    const systemPrompt = [
        'You suggest what a person opening (or continuing) a coaching conversation might ASK NEXT.',
        'Produce EXACTLY 3 short messages the USER might send.',
        '',
        'Rules:',
        '- Write in the user\'s voice (first person), not the coach\'s.',
        '- Never reveal or repeat names, emails, IDs, URLs, or other private telemetry.',
        '- Refer to activity only as a broad safe signal (for example, a lesson or decision), never as a quoted event.',
        '- Each item: 3-10 words, no trailing punctuation except a question mark.',
        isStarter
            ? '- Ground the suggestions in the user\'s RECENT ACTIVITY (from telemetry). Reference what they just did on the platform, in a natural way.'
            : '- Highly relevant to what the coach just said or asked. Where useful, also connect to the user\'s recent activity if provided.',
        // Bias the mix toward FORWARD motion. Prior versions asked for a
        // "meta/pause" option every turn which made every pill set feel
        // hedgy ("maybe I should stop"). Three forward-moving intents keep
        // energy and give the user a real choice of angles.
        '- Provide THREE forward-moving intents: (a) one concrete next-step ("How would I…", "What would I try first"), (b) one deeper reflection ("What\'s underneath this", "Why does that matter to me"), (c) one curious/open exploration ("What am I missing", "How is this connected to X").',
        '- Do NOT suggest pausing, stopping, taking a break, or asking whether to continue. The user is here to move forward.',
        '- No greetings, no thanks, no filler.',
        persona ? `- Match the tone of a "${persona}" coach\'s style (subtle) — but tone only, not risk-aversion.` : '',
        '',
        'Return ONLY a JSON array of 3 strings. No code fences, no prose.'
    ].filter(Boolean).join('\n');
    ...
    if (isStarter) {
        const safeActivity = String(activity)
            ...
            .replace(/\b[0-9a-f]{16,}\b/gi, '[event]')
            .slice(0, 6000);
        userMessage = `User's recent activity on the platform (use only broad safe signals; do not quote it):\n${safeActivity}\n\nSuggest 3 opening messages the user might send to their coach.`;
    } else if (hasActivity) {
        userMessage = `User's recent activity on the platform:\n${activity}\n\nRecent conversation:\n${transcript}\n\nSuggest 3 next user messages.`;
    } else {
    ...
    try {
        ...
        if (Array.isArray(obj)) {
            parsed = obj
                ...
                .map((s) => s.replace(/[\r\n]+/g, ' ').trim())
                .filter((s) => s.length <= 120 && !/https?:\/\/|www\.|@|\b\d{6,}\b/i.test(s))
                .slice(0, 3);
        }
    } catch (_) { /* fall back to empty on parse failure */ }

    return { suggestions: parsed, model: suggestionModel };
}

/**
 * Sync a user's activity timeline (markdown) into their vector store. Removes
 * previous "activity-*" files first so retrieval never returns stale timelines.
 * Uses the SAME store as reports so search_knowledge scope=user_data finds
 * both quiz report AND recent activity in one query.
 *
 * @param {object} opts
 * @param {string} opts.identifier    userId or objectId
 * @param {string} [opts.identifierType='userId']
 * @param {string} opts.markdown      timeline text to upload
 * @returns {Promise<{changed: boolean, storeId?, fileId?, purgedCount?}>}
 */
async function syncUserActivity({ identifier, identifierType = 'userId', markdown }) {
    if (!identifier || !markdown || !markdown.trim()) {
        return { changed: false, reason: 'missing identifier or empty markdown' };
    }
    const client = requireClient();

    // Reuse the user's store (same as reports). For guest objectId we prefix
    // to keep it distinct in the OpenAI listing.
    const storeKey = identifierType === 'objectId' ? `guest-${identifier}` : identifier;
    const storeId = await getUserVectorStoreId(storeKey);
    const filename = `activity-${new Date().toISOString().replace(/[:.]/g, '-')}.md`;

    // Purge previous activity-* files (report-* survive). We instrument this
    // heavily because the previous silent-catch swallowed the real error and
    // stale files kept accumulating in the vector store.
    let purgedCount = 0;
    let purgeErrors = [];
    try {
        const existing = await client.vectorStores.files.list(storeId, { limit: 100 });
        const files = existing.data || [];
        console.log('[vectorStore.syncUserActivity] listing store', storeId, 'returned', files.length, 'files');
        for (const f of files) {
            ...
            try {
                ...
                purgeErrors.push({ fileId: f.id, stage: 'retrieve', err: err?.message });
                continue;
            }
            if (!name.startsWith('activity-')) continue;
            try {
                // OpenAI SDK v4 uses `del` (not `delete`) for both endpoints —
                // `delete` is a reserved JS keyword.
                await client.vectorStores.files.del(storeId, f.id);
                try { await client.files.del(f.id); } catch (err) {
                    purgeErrors.push({ fileId: f.id, stage: 'files.del', err: err?.message });
                }
                purgedCount++;
            } catch (err) {
                purgeErrors.push({ fileId: f.id, filename: name, stage: 'vectorStores.files.del', err: err?.message });
            }
        }
        if (purgeErrors.length > 0) {
            console.warn('[vectorStore.syncUserActivity] purge errors:', purgeErrors);
        }
    } catch (err) {
        console.warn('[vectorStore.syncUserActivity] listing failed:', err?.message);
    }
...
async function listUserStoreFiles(storeId, { limit = 25 } = {}) {
    if (!storeId) return [];
    const client = requireClient();
    const res = await client.vectorStores.files.list(storeId, { limit });
    const files = res.data || [];
    // Enrich with filename (the list endpoint returns only IDs) so the debug
    // view is human-readable. Pull each file's metadata in parallel.
    const enriched = await Promise.all(files.map(async (f) => {
        let name = '';
        let bytes = null;
        try {
            const info = await client.files.retrieve(f.id);
            name = info?.filename || '';
