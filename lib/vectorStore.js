...
async function syncUserReport(userId, reportText) {
    ...
    try {
        ...
        for (const f of existing.data || []) {
            ...
            try {
                // Correct SDK signature: files.delete(vectorStoreId, fileId).
                // Previously called with (fileId, {vector_store_id}) which silently
                // failed, leaving stale files that polluted retrieval.
                await client.vectorStores.files.delete(storeId, f.id);
...
async function syncUserActivity({ identifier, identifierType = 'userId', markdown }) {
    ...
    try {
        ...
        for (const f of files) {
            ...
            try {
                await client.vectorStores.files.delete(storeId, f.id);
                try { await client.files.delete(f.id); } catch (err) {
                    purgeErrors.push({ fileId: f.id, stage: 'files.delete', err: err?.message });
                }
                purgedCount++;
            } catch (err) {
                purgeErrors.push({ fileId: f.id, filename: name, stage: 'vectorStores.files.delete', err: err?.message });
            }
        }
