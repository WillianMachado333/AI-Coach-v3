...
async function simulateConversation(opts, onEvent) {
    ...
    if (!seed) throw new Error('seedMessage required');
    ...
    const convo = [];
    let currentUserText = seed;
    const turns = [];
    for (let i = 0; i < maxTurns; i++) {
        if (i % 2 === 0) {
            // User turn (either the seed on i=0, or a generated follow-up).
            ...
            onEvent({ type: 'coach_turn', idx: i, text: coachText, ms });
            // If the coach was the last of our budget, stop here.
            if (i + 1 >= maxTurns) break;
