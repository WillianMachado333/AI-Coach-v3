...
<body class="min-h-screen bg-gray-50 flex items-center justify-center p-4">
  <div class="w-full max-w-sm bg-white rounded-2xl shadow-sm p-6 space-y-4 border border-gray-200">
    <div>
      ...
      <div class="text-xs text-gray-500 mt-1">Sign in with your Talent Transformation account. We never see your password.</div>
    </div>
    ${error ? `<div class="text-sm text-red-600">${escapeHtml(error)}</div>` : ''}
    ${configured
        ? '<a href="/auth/login" class="block w-full text-center bg-teal-600 hover:bg-teal-700 text-white text-sm font-semibold rounded-lg py-2">Sign in with TT account</a>'
        : '<div class="text-sm text-red-600">Sign-in is not configured on this deployment.</div>'}
  </div>
</body></html>`;
}
