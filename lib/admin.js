  ...
  <section class="relatorio">
    <div class="topo">
      <nav data-title="${escapeHtml(title)}">
        ...
        <span class="nav-sep">·</span>
        <span class="nav-group" tabindex="0">
          Observatory ▾
          <div class="nav-menu">
            <a href="/admin/sessions" data-match="Sessions">Sessions</a>
            <a href="/admin/users" data-match="User lookup">Users</a>
  ...
  (function () {
    ...
    function markActiveNav(title) {
      ...
      nav.querySelectorAll('a[data-match]').forEach((a) => {
        const matches = String(a.dataset.match || '').split(',').map(s => s.trim()).filter(Boolean);
        const t = title || '';
        a.classList.toggle('active', matches.some((m) => t.startsWith(m)));
      });
    }
...
function hubTile({ title, href, count, blurb }) {
    return `<a class="hub-tile" href="${href}">
      <div class="hub-title">${escapeHtml(title)}${count != null ? ` <span class="hub-count">${count}</span>` : ''}</div>
...
function injectedDataPage() {
    ...
    <div class="hub-grid">
      ...
      ${hubTile({ title: 'Canonical courses (planned)', href: '/admin/injected-data', blurb: 'Canonical course names + Wix URLs Erica may cite. Coming soon — today this comes from the Wix preparation payload.' })}
      ${hubTile({ title: 'Canonical quizzes (planned)', href: '/admin/injected-data', blurb: 'Canonical quiz names + Wix URLs. Coming soon — same source today.' })}
      ${hubTile({ title: 'Safety / refusal rules (planned)', href: '/admin/injected-data', blurb: 'Anchor phrases and refusal rules. Currently baked into the coach system prompt.' })}
    </div>
    <div class="painel" style="margin-top:18px">
      <h3>What belongs here</h3>
      <ul style="margin:6px 0 0 18px;color:var(--ink2);font-size:13px;line-height:1.6">
...
function courseDetailPage(courseId, savedMessage = null) {
    ...
    <div class="painel" style="border-color:rgba(208,59,59,.4)">
      ...
      <form method="POST" action="/admin/courses/${encodeURIComponent(courseId)}/delete" onsubmit="return confirm('Delete course &quot;${escapeHtml(meta.title || courseId)}&quot;? This cannot be undone through the UI.')">
    ...
    <script>
      (function () {
        ...
        // Honour ?tab=… OR #tab-… from the URL so links from the course list
        // can jump straight into the right artifact editor.
        function fromUrl() {
        ...
        if (wanted && document.querySelector('.artifact-tab[data-tab="' + wanted + '"]')) activate(wanted);
        window.addEventListener('hashchange', () => { const w = fromUrl(); if (w) activate(w); });
      })();
    </script>`;
      ...
      <div class="live-bar">
        ...
        <label style="font-size:12px;color:var(--muted);display:flex;align-items:center;gap:6px">Scenario
          <select id="liveScenario">
            ...
            <option value="/simulator-host/?erica=preview&simulator=1">Homepage (dashboard)</option>
...
async function handle(req, res) {
    ...
    if (req.method === 'GET' && (url === '/admin/realtime' || url.startsWith('/admin/realtime?'))) {
        sendPage(req, res, realtimeConfigPage(runtimeConfig.listChannels()));
        return true;
    }

    // Protected: toggle a real-time channel.
    ...
    if (req.method === 'GET' && url.match(/^\/admin\/courses\/[^/]+$/)) {
        ...
        sendPage(req, res, courseDetailPage(courseId));
        return true;
    }
    ...
    if (req.method === 'GET' && url.match(/^\/admin\/quizzes\/[^/]+$/)) {
        ...
        sendPage(req, res, quizDetailPage(quizId));
        return true;
    }
