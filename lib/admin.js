...
<script>
  (function () {
    ...
    let history = []; // in-memory only for now; server-owned history comes in a follow-up.
    ...
    async function ask(text) {
      ...
      try {
        const r = await fetch('/api/admin/studio-chat', {
          ...
          body: JSON.stringify({ history, message: text, page: location.pathname + location.search })
        });
        const data = await r.json();
        thinking.remove();
        if (data.error) { const w = el('div', 'msg erro'); w.textContent = 'error: ' + data.error; chat.appendChild(w); scrollBottom(); return; }
        history = data.history || history;
        appendAssistant(data.text || '(no response)');
      } catch (err) {
        thinking.remove();
        const w = el('div', 'msg erro'); w.textContent = 'network error: ' + err.message;
        chat.appendChild(w); scrollBottom();
      }
    }
