...
<script src="https://cdn.tailwindcss.com"></script>
<style>
  /* Palette from daily-report (validated light+dark+CVD). */
  :root { color-scheme: light dark;
    --bg:#f9f9f7; --card:#fcfcfb; --soft:#f2f1ee;
  ...
  @media (prefers-color-scheme: dark) { :root {
    --bg:#0d0d0d; --card:#1a1a19; --soft:#232322;
  ...
  * { box-sizing: border-box; }
  ...
  code, pre { font: 400 12.5px/1.5 ui-monospace,Consolas,monospace; }
  .layout { display:grid; grid-template-columns: minmax(320px, 38%) 1fr; height:100vh; }
  @media (max-width:900px) { body { overflow:auto; height:auto; } .layout { grid-template-columns: 1fr; height:auto; } }

  /* -- agent panel -- */
  .agente { display:flex; flex-direction:column; border-right:1px solid var(--line); background:var(--card); min-height:0; }
  .agente header { padding:14px 16px; border-bottom:1px solid var(--line); display:flex; align-items:baseline; gap:8px; }
  .agente header h1 { font-size:14px; margin:0; }
  .agente header span { font-size:12px; color: var(--muted); }
  .chat { flex:1; overflow-y:auto; padding:16px; display:flex; flex-direction:column; gap:14px; min-height:240px; }
  .msg { max-width:92%; }
  .msg.user { align-self:flex-end; background:var(--accent); color:#fff; padding:9px 13px; border-radius:14px 14px 4px 14px; }
  .msg.assistant { align-self:flex-start; }
  .msg.assistant .corpo { white-space:pre-wrap; }
  .msg.erro { color:var(--crit); font-size:14px; }
  .tarefas { list-style:none; margin:0 0 8px; padding:0; font-size:12.5px; }
  .tarefas li { display:flex; align-items:baseline; gap:7px; padding:2.5px 0; color:var(--ink2); }
  ...
  .tarefas .marca { color:var(--good); font-weight:700; width:11px; flex:none; }
  .tarefas .marca.falhou { color:var(--crit); }
  .tarefas .giro { width:9px; height:9px; flex:none; border-radius:50%; align-self:center;
    border:1.5px solid var(--line); border-top-color:var(--accent); animation:gira .7s linear infinite; }
  @keyframes gira { to { transform: rotate(360deg); } }
  @media (prefers-reduced-motion: reduce) { .tarefas .giro { animation-duration: 2.4s; } }
  ...
  .sugestoes button { font:inherit; font-size:13px; line-height:1.35; text-align:left;
    padding:6px 11px; color:var(--accent); background:transparent;
    border:1px solid var(--line); border-radius:14px; cursor:pointer; }
  .sugestoes button:hover { border-color:var(--accent); }
  .vazio { color:var(--muted); font-size:14px; }
  .vazio ul { padding-left:18px; margin:8px 0 0; }
  .vazio button { background:none; border:0; color:var(--accent); font:inherit; text-align:left;
  ...
  .composer textarea { flex:1; resize:none; font:inherit; color:var(--ink); background:var(--bg);
    border:1px solid var(--line); border-radius:8px; padding:9px 11px; max-height:140px; }
  .composer textarea:focus { outline:2px solid var(--accent); outline-offset:1px; }
  .icone { width:38px; height:38px; flex:none; border:1px solid var(--line); border-radius:8px;
    background:var(--bg); color:var(--ink); cursor:pointer; font-size:16px; line-height:1; }
  .enviar { background:var(--accent); color:#fff; border-color:var(--accent); font-weight:600; }

  /* -- content panel -- */
  .relatorio { overflow-y:auto; padding:20px 24px 60px; min-height:0; }
  .topo { display:flex; flex-wrap:wrap; gap:12px; align-items:center; justify-content:space-between; margin-bottom:18px; }
  .topo h2 { font-size:17px; margin:0; }
  .topo nav { display:flex; gap:14px; font-size:13px; }
  .topo nav a { color:var(--ink2); }
  .topo nav a.active { color:var(--ink); font-weight:600; }
  .topo .acoes { display:flex; gap:8px; align-items:center; }
  .botao { font:inherit; padding:7px 10px; color:var(--ink); background:var(--card);
    border:1px solid var(--line); border-radius:6px; cursor:pointer; text-decoration:none; display:inline-block; }
  .cards { display:grid; grid-template-columns:repeat(auto-fit, minmax(158px, 1fr)); gap:12px; margin-bottom:20px; }
  .card { background:var(--card); border:1px solid var(--line); border-radius:10px; padding:13px 15px;
    box-shadow:0 1px 2px var(--ring); display:flex; flex-direction:column; gap:2px; }
  .card .rot { font-size:10.5px; font-weight:600; text-transform:uppercase; letter-spacing:.06em; color:var(--muted); }
  .card .val { font-size:27px; font-weight:700; letter-spacing:-.02em; line-height:1.15; }
  .card .var { font-size:12px; color:var(--muted); }
  .grafico { background:var(--card); border:1px solid var(--line); border-radius:10px;
    padding:14px 16px 10px; box-shadow:0 1px 2px var(--ring); min-width:0; }
  .grafico h3 { font-size:13px; margin:0 0 2px; font-weight:600; }
  .grafico svg { display:block; overflow:visible; max-width:100%; }
  .painel { background:var(--card); border:1px solid var(--line); border-radius:10px; padding:14px 16px; box-shadow:0 1px 2px var(--ring); margin-bottom:14px; }
  .painel h3 { font-size:13px; margin:0 0 8px; font-weight:600; }
  ...
  input, textarea, select { font:inherit; color:var(--ink); background:var(--bg); border:1px solid var(--line); border-radius:6px; padding:6px 9px; }
  .titulo { display:flex; justify-content:space-between; align-items:baseline; margin-bottom:12px; }
  .titulo h2 { font-size:17px; margin:0; }
  .titulo .meta { font-size:12px; color:var(--muted); }
...
<div class="layout">
  <section class="agente">
    <header><h1>Coach Studio</h1><span>your co-worker</span></header>
    <div class="chat" id="chat">
      <div class="vazio" id="vazio">
        <p>Ask about sessions, personas, quality signals — I read the logs before answering.</p>
        <ul>
  ...
  <section class="relatorio">
    <div class="topo">
      <nav>
        ...
        <a href="/admin/users">User lookup</a>
        <a href="/admin/simulator">Simulator</a>
        <a href="/admin/frameworks">Frameworks</a>
...
<script>
  (function () {
    ...
    let currentMarker = null; // { text, source: url }
    ...
    function buildChart(spec) {
      ...
      const max = Math.max(1, ...allValues.filter((v) => Number.isFinite(v)));
    ...
    async function ask(text) {
      ...
      wrap.appendChild(tasks); wrap.appendChild(body);
      ...
      try {
        ...
        const r = await fetch('/api/admin/agent', {
          method: 'POST', credentials: 'include',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ message: text, page: location.pathname + location.search, marker: markerToSend })
        });
        if (!r.ok || !r.body) {
    ...
    marcarBtn.addEventListener('click', () => {
      ...
      if (!text) return;
      currentMarker = { text, source: location.pathname };
      renderMarkerStrip();
    ...
    send.addEventListener('click', () => { const v = ta.value.trim(); if (!v) return; ta.value = ''; ta.style.height = ''; ask(v); });
...
function indexPage(session) {
    ...
    const tile = (rot, val, sub) => `<div class="card"><div class="rot">${escapeHtml(rot)}</div><div class="val">${escapeHtml(String(val))}</div><div class="var">${escapeHtml(sub || '')}</div></div>`;
    const signal = (rot, val, hint, tone) => `<div class="card"><div class="rot">${escapeHtml(rot)}</div><div class="val" style="color:${tone || 'inherit'}">${escapeHtml(String(val))}</div><div class="var">${escapeHtml(hint || '')}</div></div>`;
    ...
    const body = `
    <div class="titulo"><h2>Overview</h2><span class="meta">Signed in as admin · session ${new Date((session.iat||0)*1000).toISOString().slice(11,16)} UTC</span></div>

    <div class="cards">
      ${tile('Last 24h', m.volume.last24h.sessions, m.volume.last24h.turns + ' turns · ' + m.volume.last24h.tools + ' tools')}
      ${tile('Last 7 days', m.volume.last7d.sessions, m.volume.last7d.turns + ' turns · ' + m.volume.last7d.tools + ' tools')}
      ${tile('Last 30 days', m.volume.last30d.sessions, m.volume.last30d.turns + ' turns · ' + m.volume.last30d.tools + ' tools')}
      ${signal('Rage close', m.qualitySignals.rageClose, 'short sessions ending on user', m.qualitySignals.rageClose > 0 ? 'var(--warn)' : null)}
      ${signal('Long silence', m.qualitySignals.longSilenceCount, 'gaps > 90s', m.qualitySignals.longSilenceCount > 5 ? 'var(--warn)' : null)}
      ${signal('Tool failures', m.qualitySignals.toolFailureCount, 'sessions with ≥ 1 error', m.qualitySignals.toolFailureCount > 0 ? 'var(--crit)' : null)}
    </div>
