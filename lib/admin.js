  ...
  <section class="relatorio">
    <div class="topo">
      <nav data-title="${escapeHtml(title)}">
        ...
        <a href="/admin/frameworks" data-match="Persona">Personas</a>
        <a href="/admin/courses" data-match="Course">Courses</a>
        <a href="/admin/quizzes" data-match="Quiz">Quizzes</a>
...
function coursesListPage(courses, message = null) {
    ...
    for (const c of courses) {
        ...
        const artifactCards = artifactRows.map((a) => {
            const badge = a.missing
                ? '<span style="color:var(--muted);font-size:11px">no file</span>'
                : (a.source === 'overlay'
                    ? '<span style="color:var(--accent);font-size:11px;font-weight:700">edited</span>'
                    : '<span style="color:var(--muted);font-size:11px">default</span>');
            const info = a.missing ? '' : `<div style="font-size:11px;color:var(--muted);margin-top:2px">${a.chars.toLocaleString()} chars · <code style="font-size:10.5px">${a.hash}…</code></div>`;
            return `<a class="artifact-card" href="/admin/courses/${encodeURIComponent(c.course_id)}/${encodeURIComponent(a.key)}"
...
function quizDetailPage(quizId, savedMessage = null) {
    ...
    <div class="painel">
      ...
      <form method="POST" action="/admin/quizzes/${encodeURIComponent(quizId)}/meta" style="display:flex;flex-direction:column;gap:10px">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          ...
          <label style="display:flex;flex-direction:column;gap:4px;font-size:12px;color:var(--muted)">Source URL (optional)
            <input name="source" value="${escapeHtml(q.source || '')}" placeholder="https://www.talenttransformation.com/…">
          </label>
        </div>
        <label style="display:flex;flex-direction:column;gap:4px;font-size:12px;color:var(--muted)">Description (short, shown on the quiz list)
          <textarea name="description" rows="2">${escapeHtml(q.description || '')}</textarea>
        </label>
        <label style="display:flex;flex-direction:column;gap:4px;font-size:12px;color:var(--muted)">Purpose (what this quiz measures — used by Erica to explain the quiz to learners)
    ...
    <div class="painel">
      ...
      <form method="POST" action="/admin/quizzes/${encodeURIComponent(quizId)}/content" style="display:flex;flex-direction:column;gap:10px">
        ...
        <label style="display:flex;flex-direction:column;gap:4px;font-size:12px;color:var(--muted)">Reason (audit)
          <input name="reason" placeholder="e.g. Eric: update question 3 wording">
        </label>
