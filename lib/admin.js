 ...
 *   ADMIN_USERNAME       optional allowed username
 *   ADMIN_EMAIL          optional allowed email
 *   ADMIN_LOGIN          optional allowed identity
 *   ADMIN_ALLOWED_USERS  optional comma/semicolon/newline-separated allow-list
...
function makeSessionCookie({ identity }) {
    if (!identity) throw new Error('makeSessionCookie: identity is required');
...
<body class="min-h-screen bg-gray-50 flex items-center justify-center p-4">
  <div class="w-full max-w-sm bg-white rounded-2xl shadow-sm p-6 space-y-4 border border-gray-200">
    <div>
      ...
      <div class="text-xs text-gray-500 mt-1">${adminEnvironmentLabel()} · Sign in to manage the AI Coach V3 prototype.</div>
    </div>
    ...
    <form method="POST" action="/admin/login" class="space-y-4" id="admin-login-form">
      <label class="block text-sm text-gray-700">
        <span class="block mb-1 font-medium">Email or username</span>
        <input class="w-full" type="text" name="identity" autocomplete="username" required autofocus>
      </label>
      <label class="block text-sm text-gray-700">
        <span class="block mb-1 font-medium">Password</span>
        <input class="w-full" type="password" name="password" autocomplete="current-password" required>
      </label>
      <p id="login-status" class="text-xs text-gray-500" aria-live="polite" hidden>Signing in…</p>
      <button type="submit" class="w-full bg-teal-600 hover:bg-teal-700 disabled:opacity-60 text-white text-sm font-semibold rounded-lg py-2">Sign in</button>
    </form>
  </div>
  <script>
...
async function handle(req, res) {
    ...
    if (req.method === 'POST' && url === '/admin/login') {
        ...
        const result = authenticateAdmin({ identity, password });
        if (result.ok) {
            res.writeHead(302, {
                Location: '/admin/',
                'Set-Cookie': makeSessionCookie({ identity: result.identity }),
            });
            res.end();
            ...
            res.end(denialPage({
                ...
                reason: 'This email or username is not authorized to access the AI Coach V3 Admin surface.'
            }));
        } else {
