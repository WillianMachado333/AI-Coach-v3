...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* -- agent panel -- */
  /* Blueprint-navy backdrop: soft radial glow at upper-left, faint teal grid,
     dots at the intersections, two whispered diagonal "scratch" strokes.
     Reproduces the reference in pure CSS so we don't pay an image request. */
  .agente { display:flex; flex-direction:column; border-right:1px solid var(--line);
    background:
      linear-gradient(115deg, transparent 0 44.9%, rgba(50,197,229,.045) 44.95%, rgba(50,197,229,.045) 45.25%, transparent 45.3%),
      linear-gradient(122deg, transparent 0 67.9%, rgba(50,197,229,.035) 67.95%, rgba(50,197,229,.035) 68.25%, transparent 68.3%),
      radial-gradient(680px 520px at 22% 34%, rgba(50, 197, 229, 0.16), transparent 65%),
      linear-gradient(180deg, #06171f 0%, #071D26 40%, #0a2635 100%);
    color: var(--navy-ink);
  ...
  .agente::before {
    ...
    background-image:
      ...
      linear-gradient(0deg, transparent 0 71px, rgba(50,197,229,.055) 71px 72px),
      linear-gradient(90deg, transparent 0 71px, rgba(50,197,229,.055) 71px 72px);
    background-size: 72px 72px, 72px 72px, 72px 72px;
    background-position: 0 0;
    mix-blend-mode: screen;
    opacity:.55;
    mask-image: radial-gradient(ellipse 110% 90% at 30% 40%, black 0%, rgba(0,0,0,.75) 60%, transparent 100%);
    -webkit-mask-image: radial-gradient(ellipse 110% 90% at 30% 40%, black 0%, rgba(0,0,0,.75) 60%, transparent 100%);
  }
  .agente > * { position:relative; z-index:1; }
  ...
  .agente header.agent-id {
    background: linear-gradient(180deg, rgba(4,14,20,.55), rgba(4,14,20,.05));
  ...
  /* parent .agente already carries the blueprint grid — no local grid needed */
  .agente header.agent-id::after { content:none; }
  ...
  /* -- coach panel: light-on-navy overrides -- */
  .agente .msg.assistant .corpo { color: var(--navy-ink); }
  .agente .msg.assistant .tarefas li { color: rgba(219,230,235,.78); }
  .agente .msg.assistant .tarefas li.done { color: rgba(143,166,177,.7); }
  .agente .msg.assistant .tarefas li em { color: var(--navy-muted); }
  .agente .msg.assistant .tarefas .marca { color: #4ade80; }
  .agente .msg.assistant .tarefas .marca.falhou { color: #fca5a5; }
  .agente .msg.assistant .tarefas .giro { border-color: rgba(255,255,255,.14);
    border-top-color: #32C5E5; }
  .agente .msg.assistant .passos { color: var(--navy-muted); }
  .agente .msg.assistant .passos > div { border-left-color: rgba(255,255,255,.10); }
  .agente .msg.erro { color:#fca5a5; background:rgba(208,59,59,.14); }
  ...
  .agente .sugestoes button:hover {
    background: rgba(50,197,229,.14);
    border-color: rgba(50,197,229,.40);
    color:#e6f7fb;
  }

  .agente .vazio { color: var(--navy-muted); }
  .agente .vazio p { color: var(--navy-ink); }
  .agente .vazio button {
    color: var(--navy-ink);
    background: rgba(255,255,255,.045);
    border: 1px solid rgba(255,255,255,.10);
  ...
  .agente .vazio button:hover {
    background: rgba(50,197,229,.12);
    border-color: rgba(50,197,229,.35);
  }

  .agente .composer {
    border-top: 1px solid rgba(255,255,255,.08);
    background: linear-gradient(180deg, rgba(6,20,28,.55), rgba(6,20,28,.85));
    backdrop-filter: blur(8px);
    -webkit-backdrop-filter: blur(8px);
  }
  .agente .composer textarea {
    color: var(--navy-ink);
    background: rgba(255,255,255,.04);
    border-color: rgba(255,255,255,.10);
  }
  .agente .composer textarea::placeholder { color: rgba(143,166,177,.7); }
  .agente .composer textarea:focus {
    border-color: #32C5E5;
    box-shadow: 0 0 0 3px rgba(50,197,229,.18);
  }
  .agente .composer .icone {
    color: var(--navy-muted);
    background: rgba(255,255,255,.045);
    border-color: rgba(255,255,255,.10);
  }
  .agente .composer .icone:hover { color: var(--navy-ink); border-color: rgba(255,255,255,.22); }
  /* .enviar keeps its teal gradient — still reads perfectly on navy */
