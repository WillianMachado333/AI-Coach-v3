 ...
 *
 * Env vars:
 *   ADMIN_PASSWORD       shared prototype admin password
...
const { authenticatePassword } = require('./adminAuth');
...
<body class="min-h-screen bg-slate-50 flex items-center justify-center p-4">
  <div class="w-full max-w-sm bg-white rounded-2xl shadow-sm p-6 space-y-5 border border-gray-200">
    <div>
      ...
      <div class="text-xs text-slate-500 mt-1">${adminEnvironmentLabel()} · Secure local access for the AI Coach V3 prototype.</div>
    </div>
    ...
    <form method="POST" action="/admin/login" class="space-y-4" id="admin-login-form">
      <label class="block text-sm text-gray-700">
        ...
        <input class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-teal-600 focus:ring-2 focus:ring-teal-100 outline-none" type="email" name="identity" autocomplete="username" required autofocus>
      </label>
      <label class="block text-sm text-gray-700">
        <span class="block mb-1 font-medium">Password</span>
        <input class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-teal-600 focus:ring-2 focus:ring-teal-100 outline-none" type="password" name="password" autocomplete="current-password" required>
      </label>
      <p id="login-status" class="text-xs text-gray-500" aria-live="polite" hidden>Signing in…</p>
      <button type="submit" class="w-full bg-teal-600 hover:bg-teal-700 disabled:opacity-60 text-white text-sm font-semibold rounded-lg py-2.5">Sign in</button>
    </form>
    <p class="text-xs leading-5 text-slate-500">Accounts are registered locally after a successful sign-in. Admin access is granted by the local role, not by a Wix badge.</p>
  </div>
  <script>
...
async function handle(req, res) {
    ...
    // ---- Public: authenticate and create a local session ------------------
    if (req.method === 'POST' && url === '/admin/login') {
        const body = await readBody(req);
        const identity = extractField(body, 'identity')
            || extractField(body, 'username')
            || extractField(body, 'email');
        const password = extractField(body, 'password');
        const result = authenticatePassword({ identity, password });
        const user = result.ok ? usersStore.recordSuccessfulLogin(result.identity) : null;
        if (result.ok && user && user.role === 'admin' && user.status === 'active') {
            res.writeHead(302, {
                Location: '/admin/',
                'Set-Cookie': makeSessionCookie({ identity: user.identity, userId: user.id }),
            });
            res.end();
        } else if (result.ok) {
            res.writeHead(403, {
            ...
            res.end(denialPage({
                email: user?.identity || result.identity,
                reason: 'This local account is not marked as an administrator.'
            }));
        } else {
            const status = result.code === 'config_error' ? 500 : 401;
            const message = result.code === 'config_error'
                ? 'Admin sign-in is not configured for this environment.'
                : 'Invalid email/username or password.';
            res.writeHead(status, {
                'Content-Type': 'text/html; charset=utf-8',
                'Set-Cookie': clearSessionCookie(),
            });
            res.end(loginPage({ error: message }));
        }
        return true;
    }
