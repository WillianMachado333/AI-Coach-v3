...
<body class="min-h-screen bg-gray-50 flex items-center justify-center p-4">
  <div class="w-full max-w-sm bg-white rounded-2xl shadow-sm p-6 space-y-4 border border-gray-200">
    <div>
      ...
      <div class="text-xs text-gray-500 mt-1">Sign in with your Talent Transformation account. Wix handles the sign-in — we never see your password.</div>
    </div>
    ${error ? `<div class="text-sm text-red-600">${escapeHtml(error)}</div>` : ''}
    ${configured
        ? `<a href="/auth/login${nextParam}" class="block w-full text-center bg-teal-600 hover:bg-teal-700 text-white text-sm font-semibold rounded-lg py-2">Sign in with Wix</a>`
        : `<div class="text-sm text-red-600">WIX_CLIENT_ID is not set on this deployment. Sign-in cannot start until it is configured.</div>`}
  </div>
</body></html>`;
}
...
<body class="min-h-screen bg-gray-50 flex items-center justify-center p-4">
  <div class="w-full max-w-md bg-white rounded-2xl shadow-sm p-6 space-y-4 border border-gray-200">
    ...
    ${failed
        ? '<p class="text-xs text-gray-500">This is not necessarily a permissions problem — Wix may be unavailable, or the deployment may be missing WIX_CLIENT_SECRET. Try again in a minute.</p>'
        : `<p class="text-xs text-gray-500">Ask the operator (Willian) to assign you the "${escapeHtml(wixBadges.ADMIN_BADGE)}" badge in the Wix dashboard. Access takes effect within a minute.</p>`}
