/*
 * AI Coach V3 Admin — custom backend login.
 *
 * The login form is rendered on this surface and POSTs credentials to the
 * same-origin backend. A signed, httpOnly cookie maintains the session for
 * protected Admin pages and API routes.
 *
 * Env vars:
 *   WIX_CLIENT_ID        Talent Transformation Wix Headless OAuth client id
 *   AI_COACH_ORIGIN      optional public origin used in the registered callback URI
 *   INITIAL_ADMIN_EMAILS optional comma/semicolon/newline-separated bootstrap admins
...
const { beginWixLogin, completeWixLogin } = require('./wixAuth');
const { aiCoachEnvironmentLabel } = require('./environmentLabel');

const COOKIE_NAME = 'admin_session';
const WIX_PKCE_COOKIE = 'admin_wix_pkce';
const TTL_SECONDS = 4 * 60 * 60; // 4h
const ADMIN_PRODUCT_NAME = 'AI Coach V3 Admin';

function adminEnvironmentLabel() {
    return aiCoachEnvironmentLabel(process.env.AI_COACH_ENVIRONMENT);
}
...
function makeSessionCookie({ identity, userId }) {
    if (!identity || !userId) throw new Error('makeSessionCookie: identity and userId are required');
    ...
    // SameSite=Lax prevents cross-site POSTs from riding the cookie.
...
function wixPkceCookie(pkce) {
    const secure = process.env.NODE_ENV === 'production' ? '; Secure' : '';
    return `${WIX_PKCE_COOKIE}=${b64urlEncode(JSON.stringify(pkce))}; Path=/admin; Max-Age=900; HttpOnly${secure}; SameSite=Lax`;
}
function clearWixPkceCookie() {
    const secure = process.env.NODE_ENV === 'production' ? '; Secure' : '';
    return `${WIX_PKCE_COOKIE}=; Path=/admin; Max-Age=0; HttpOnly${secure}; SameSite=Lax`;
}
function readWixPkce(cookie) {
    try { return JSON.parse(b64urlDecode(cookie).toString('utf8')); } catch (_) { return null; }
}
...
<html><head>
<meta charset="utf-8">
<title>${ADMIN_PRODUCT_NAME} — Sign in</title>
<meta name="viewport" content="width=device-width,initial-scale=1">
<link rel="icon" type="image/svg+xml" href="${FAVICON_HREF}">
<script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="min-h-screen bg-slate-50 flex items-center justify-center p-4">
  <div class="w-full max-w-sm bg-white rounded-2xl shadow-sm p-6 space-y-5 border border-gray-200">
    <div>
      <div class="text-lg font-semibold text-slate-950">${ADMIN_PRODUCT_NAME}</div>
      <div class="text-xs text-slate-500 mt-1">${adminEnvironmentLabel()} · Sign in with your Talent Transformation account.</div>
    </div>
    ${error ? `<div class="text-sm text-red-600">${escapeHtml(error)}</div>` : ''}
    ${process.env.WIX_CLIENT_ID ? `<form method="POST" action="/admin/auth/login" id="admin-login-form"><button type="submit" class="w-full bg-teal-600 hover:bg-teal-700 disabled:opacity-60 text-white text-sm font-semibold rounded-lg py-2.5">Continue with Talent Transformation</button></form><p id="login-status" class="text-xs text-gray-500" aria-live="polite" hidden>Taking you to sign in…</p><p class="text-xs leading-5 text-slate-500">Talent Transformation verifies your account. This app records the verified sign-in and uses its local role for admin access.</p>` : `<p class="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800">Talent Transformation sign-in is not configured for this environment.</p>`}
  </div>
  <script>
    ...
    if (form) form.addEventListener('submit', function () {
      ...
      button.textContent = 'Taking you to sign in…';
      status.hidden = false;
...
// Kept plain HTML rather than shellChrome() because the shell itself is behind
// the gate this page explains.
function denialPage({ email, reason }) {
    const title = 'Access denied';
    return `<!doctype html>
<html><head>
<meta charset="utf-8">
<title>${ADMIN_PRODUCT_NAME} — ${escapeHtml(title)}</title>
<meta name="viewport" content="width=device-width,initial-scale=1">
<link rel="icon" type="image/svg+xml" href="${FAVICON_HREF}">
<script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="min-h-screen bg-gray-50 flex items-center justify-center p-4">
  <div class="w-full max-w-md bg-white rounded-2xl shadow-sm p-6 space-y-4 border border-gray-200">
    ...
    <p class="text-xs text-gray-500">Use an authorized email or username, or contact the AI Coach V3 owner.</p>
...
function userLookupPage(input, users = [], message = '') {
    const registryRows = users.map((user) => `<tr class="border-t border-gray-100"><td class="px-3 py-2 font-mono text-xs">${escapeHtml(user.identity)}</td><td class="px-3 py-2"><span class="rounded-full px-2 py-0.5 text-xs ${user.role === 'admin' ? 'bg-teal-100 text-teal-800' : 'bg-gray-100 text-gray-700'}">${escapeHtml(user.role)}</span></td><td class="px-3 py-2 text-xs text-gray-500">${user.loginCount || 0}</td><td class="px-3 py-2 text-xs text-gray-500">${escapeHtml(user.lastLoginAt || '—')}</td><td class="px-3 py-2"><form method="POST" action="/admin/users/role" class="flex gap-2"><input type="hidden" name="identity" value="${escapeHtml(user.identity)}"><select name="role" class="rounded border border-gray-300 px-1 py-1 text-xs"><option value="user" ${user.role === 'user' ? 'selected' : ''}>User</option><option value="admin" ${user.role === 'admin' ? 'selected' : ''}>Admin</option></select><button class="rounded bg-gray-800 px-2 py-1 text-xs text-white">Save</button></form></td></tr>`).join('');
    const body = `
    <section class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
      <div class="p-4 border-b border-gray-200"><div class="font-semibold text-sm">Local access registry</div><p class="text-xs text-gray-500 mt-1">Roles are stored in the app volume. A successful password check records a user; only the local <code>admin</code> role can enter this console.</p>${message ? `<p class="text-xs text-teal-700 mt-2">${escapeHtml(message)}</p>` : ''}</div>
      <div class="overflow-x-auto"><table class="w-full text-sm"><thead class="bg-gray-50 text-left text-xs text-gray-600"><tr><th class="px-3 py-2">Identity</th><th class="px-3 py-2">Role</th><th class="px-3 py-2">Logins</th><th class="px-3 py-2">Last sign-in</th><th class="px-3 py-2">Access</th></tr></thead><tbody>${registryRows || '<tr><td colspan="5" class="px-3 py-5 text-center text-sm text-gray-500">No registered users yet.</td></tr>'}</tbody></table></div>
    </section>
...
async function handle(req, res) {
    ...
    // Seed the local registry as soon as the admin surface is opened, rather
    // than waiting for a successful login. This makes the bootstrap admin a
    // real durable record before they submit the form.
    usersStore.ensureBootstrapUsers();

    // Parse cookies once.
    const cookies = parseCookie(req.headers.cookie);
    let session = verifySession(cookies[COOKIE_NAME]);
    if (session) {
        const user = usersStore.get(session.u);
        if (!user || user.status !== 'active' || user.role !== 'admin' || (session.uid && session.uid !== user.id)) session = null;
    }

    // ---- Public: custom sign-in form --------------------------------------
    if (req.method === 'GET' && (url === '/admin/login' || url.startsWith('/admin/login?'))) {
        const params = new URL(url, 'http://x').searchParams;
        const errorCode = params.get('e');
        const messages = {
            session_expired: 'Your session expired. Sign in again.',
            access_denied: 'This account is not authorized to access the Admin surface.',
            wix_error: 'Talent Transformation could not complete the sign-in. Please try again.'
        };
    ...
    // ---- Public: start Wix/Talent Transformation OAuth with PKCE ---------
    if (req.method === 'POST' && url === '/admin/auth/login') {
        try {
            const started = await beginWixLogin();
            res.writeHead(302, { Location: started.url, 'Set-Cookie': wixPkceCookie(started.pkce) });
        } catch (error) {
            console.warn('[admin] Wix login start failed:', error?.message || error);
            res.writeHead(302, { Location: '/admin/login?e=wix_error', 'Set-Cookie': clearWixPkceCookie() });
        }
        res.end();
        return true;
    }

    // Wix returns the authorization code in the URL fragment, which never
    // reaches Node. This page moves it to a same-origin POST body and clears it.
    if (req.method === 'GET' && (url === '/admin/auth/callback' || url.startsWith('/admin/auth/callback?'))) {
        res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
        res.end(`<!doctype html><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Signing in…</title><p>Signing in…</p><script>(function(){var p=new URLSearchParams(location.hash.slice(1)),c=p.get('code'),s=p.get('state');history.replaceState(null,'',location.pathname);if(!c||!s){location.replace('/admin/login?e=wix_error');return}var f=document.createElement('form');f.method='POST';f.action='/admin/auth/complete';[['code',c],['state',s]].forEach(function(x){var i=document.createElement('input');i.type='hidden';i.name=x[0];i.value=x[1];f.appendChild(i)});document.body.appendChild(f);f.submit()}())</script>`);
        return true;
    }

    // ---- Public: exchange verified Wix identity for a local admin session --
    if (req.method === 'POST' && url === '/admin/auth/complete') {
        const body = await readBody(req);
        try {
            const identity = await completeWixLogin({
                code: extractField(body, 'code'), state: extractField(body, 'state'), pkce: readWixPkce(cookies[WIX_PKCE_COOKIE]),
            });
            const user = usersStore.recordSuccessfulLogin(identity.email);
            if (!user || user.role !== 'admin' || user.status !== 'active') {
                res.writeHead(403, { 'Content-Type': 'text/html; charset=utf-8', 'Set-Cookie': [clearWixPkceCookie(), clearSessionCookie()] });
                res.end(denialPage({ email: identity.email, reason: 'This local account is not marked as an administrator.' }));
            } else {
                res.writeHead(302, { Location: '/admin/', 'Set-Cookie': [clearWixPkceCookie(), makeSessionCookie({ identity: user.identity, userId: user.id })] });
                res.end();
            }
        } catch (error) {
            console.warn('[admin] Wix login completion failed:', error?.message || error);
            res.writeHead(302, { Location: '/admin/login?e=wix_error', 'Set-Cookie': [clearWixPkceCookie(), clearSessionCookie()] });
            res.end();
        }
        return true;
    }
    ...
    // ---- Public: logout -------------------------------------------------
    // POST-only so a stray <img src="/admin/logout"> can't sign someone out.
    ...
    // A missing or expired cookie returns to the same-origin custom form.
    if (!session) {
    ...
    // Protected: change a local role. The store protects the last active admin.
    if (req.method === 'POST' && url === '/admin/users/role') {
        const body = await readBody(req);
        try {
            const user = usersStore.setRole(extractField(body, 'identity'), extractField(body, 'role'), session.u);
            auditLog.append({ actor: session.u, action: 'users.role.update', target: user.identity, meta: { role: user.role } });
            res.writeHead(302, { Location: '/admin/users?m=role_saved' });
        } catch (error) {
            res.writeHead(302, { Location: '/admin/users?m=' + encodeURIComponent(error.message || 'role_update_failed') });
        }
        res.end();
        return true;
    }

    // Protected: local access registry plus legacy diagnostic lookup.
    if (req.method === 'GET' && (url === '/admin/users' || url.startsWith('/admin/users?'))) {
        const params = new URL(url, 'http://x').searchParams;
        const q = params.get('q');
        const users = usersStore.list();
        if (!q) {
            sendPage(req, res, userLookupPage('', users, params.get('m') || ''));
            return true;
        }
        try {
 ...
 */
// Used by /api/admin/* endpoints in server.js. Returns the verified local
// session payload on success, null on any failure — callers just do
// `if (!sess) return 401`.
async function requireAdminSession(req) {
    ...
    const user = usersStore.get(session.u);
    if (!user || user.status !== 'active' || user.role !== 'admin' || (session.uid && session.uid !== user.id)) return null;
    return { ...session, identity: user.identity, role: user.role };
}

module.exports = { handle, signSession, verifySession, requireAdminSession, adminEnvironmentLabel };
