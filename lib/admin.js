...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* Draggable divider between agent panel + content panel. */
  #splitter { grid-column:2; grid-row:1; width:6px; margin-left:-3px; cursor:col-resize;
    background:transparent; position:relative; z-index:10; }
  #splitter::after { content:""; position:absolute; left:2px; right:2px; top:0; bottom:0;
  ...
  (function () {
    ...
    (function initSplitter() {
      ...
      function applyFraction(frac) {
        ...
        const leftPx = Math.max(MIN_LEFT_PX, Math.min(cw - MIN_RIGHT_PX, frac * cw));
        layout.style.gridTemplateColumns = leftPx + 'px 1fr';
      }
      const saved = parseFloat(localStorage.getItem(STORAGE_KEY) || '');
      if (Number.isFinite(saved) && saved > 0.15 && saved < 0.7) applyFraction(saved);
      let dragging = false;
      ...
      const stop = (e) => {
        ...
        // Persist the current fraction (clamped inside applyFraction).
        const cw = layout.clientWidth;
        const leftPx = parseFloat(layout.style.gridTemplateColumns) || 0;
        if (cw && leftPx) localStorage.setItem(STORAGE_KEY, String(leftPx / cw));
      };
...
function simulatorPage(prefill = {}) {
    ...
    const defaultGuardrails = prefill.guardrails
        || '- Lead with emotional validation before naming next steps.\n- Offer small, concrete next steps rather than vague reassurance.\n- Avoid urgency language and imperatives ("you must", "you should").\n- If the user is spiraling, name it and slow down.\n- Never diagnose; ground reflections in what the user just said.';
    const defaultUserPersona = prefill.userPersona
        || 'You are a mid-career professional feeling stuck in your current role. You are considering a startup offer but scared of the risk. You alternate between excitement and second-guessing.';
    ...
    const body = `
    <div class="titulo"><h2>Simulator</h2><span class="meta">Run coaching turns against alternative personas — single reply or a full multi-turn conversation.</span></div>

    <div class="painel">
      <h3>Multi-turn conversation</h3>
      <p style="font-size:13px;color:var(--ink2);margin:0 0 12px">
        Compose a coach persona AND a user persona, then let them talk for N turns. This is the "real" simulator —
        streams live, one turn at a time, so you see the exchange as it happens. Use it before shipping a new coach
        prompt to sanity-check drift, redundancy, escalation, or refusal patterns.
      </p>
      <form id="convForm" style="display:flex;flex-direction:column;gap:10px">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          <label style="display:flex;flex-direction:column;gap:4px;font-size:12px;color:var(--muted)">
            Coach name
          ...
          <label style="display:flex;flex-direction:column;gap:4px;font-size:12px;color:var(--muted)">
            Max turns (user + coach)
            <input name="maxTurns" type="number" min="2" max="20" step="2" value="${escVal(prefill.maxTurns || '8')}">
          </label>
        </div>
        <label style="display:flex;flex-direction:column;gap:4px;font-size:12px;color:var(--muted)">
          Coach guardrails / persona rules
          <textarea name="guardrails" rows="6" style="font:12.5px ui-monospace,Consolas,monospace">${escVal(defaultGuardrails)}</textarea>
        </label>
        <label style="display:flex;flex-direction:column;gap:4px;font-size:12px;color:var(--muted)">
          Extra coach directive (optional)
          <input name="extraDirective" value="${escVal(prefill.extraDirective || '')}" placeholder="e.g. 'Push harder for a decision this turn'">
        </label>
        <label style="display:flex;flex-direction:column;gap:4px;font-size:12px;color:var(--muted)">
          User persona (drives the fake user)
          <textarea name="userPersona" rows="4" style="font:12.5px ui-monospace,Consolas,monospace">${escVal(defaultUserPersona)}</textarea>
        </label>
        <label style="display:flex;flex-direction:column;gap:4px;font-size:12px;color:var(--muted)">
          Seed message (first user turn)
          <textarea name="seedMessage" rows="2">${escVal(defaultSeed)}</textarea>
        </label>
        <div style="display:flex;gap:8px;align-items:center">
          <button class="botao" style="background:var(--accent);color:#fff;border-color:transparent;font-weight:600" id="convRun">Run conversation</button>
          <span id="convStatus" style="font-size:12px;color:var(--muted)"></span>
        </div>
      </form>
      <div id="convTranscript" style="margin-top:16px;display:flex;flex-direction:column;gap:10px"></div>
    </div>

    <details style="margin-top:12px">
      <summary style="cursor:pointer;padding:8px 0;color:var(--ink2);font-size:13px">Single-turn simulator (legacy — one user message → one reply)</summary>
      <div class="painel" style="margin-top:8px">
        <form id="simForm" style="display:flex;flex-direction:column;gap:10px">
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
            <label style="display:flex;flex-direction:column;gap:4px;font-size:12px;color:var(--muted)">
              Persona name
              <input name="personaName" value="${escVal(prefill.personaName || 'Erica')}">
            </label>
            <label style="display:flex;flex-direction:column;gap:4px;font-size:12px;color:var(--muted)">
              Extra directive
              <input name="extraDirective" value="${escVal(prefill.extraDirective || '')}" placeholder="e.g. 'Be more direct'">
            </label>
          </div>
          <label style="display:flex;flex-direction:column;gap:4px;font-size:12px;color:var(--muted)">
            Guardrails
            <textarea name="guardrails" rows="4" style="font:12.5px ui-monospace,Consolas,monospace">${escVal(defaultGuardrails)}</textarea>
          </label>
          <label style="display:flex;flex-direction:column;gap:4px;font-size:12px;color:var(--muted)">
            User message
            <textarea name="userMessage" rows="3">${escVal(prefill.userMessage || 'I feel stuck. I have a big decision to make.')}</textarea>
          </label>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
            <label style="display:flex;flex-direction:column;gap:4px;font-size:12px;color:var(--muted)">
              Replay session id
              <input name="replaySessionId" value="${escVal(prefill.replaySessionId || '')}" placeholder="s-…" style="font:12.5px ui-monospace,Consolas,monospace">
            </label>
            <label style="display:flex;flex-direction:column;gap:4px;font-size:12px;color:var(--muted)">
              Replay turn index
              <input name="replayTurnIndex" type="number" min="0" value="${escVal(prefill.replayTurnIndex || '')}">
            </label>
          </div>
          <button class="botao" style="background:var(--accent);color:#fff;border-color:transparent;font-weight:600" id="simRun">Simulate one turn</button>
        </form>
        <div id="results" style="margin-top:12px;display:none">
          <div id="original-block" style="display:none;margin-bottom:10px">
            ...
            <div id="original" style="margin-top:4px;font-size:13.5px;background:var(--soft);border:1px solid var(--line);border-radius:8px;padding:10px;white-space:pre-wrap"></div>
          </div>
          <div>
            <div style="font-size:11px;font-weight:600;color:var(--accent);text-transform:uppercase;letter-spacing:.05em">Simulated response</div>
            <div id="simulated" style="margin-top:4px;font-size:13.5px;background:var(--accent-tint);border:1px solid var(--line);border-radius:8px;padding:10px;white-space:pre-wrap"></div>
            <div id="meta" style="margin-top:4px;font-size:11px;color:var(--muted)"></div>
          </div>
          <details style="margin-top:8px">
            <summary style="cursor:pointer;font-size:12px;color:var(--muted)">Instructions used</summary>
            <pre id="instr" style="margin-top:6px;font-size:11px;background:var(--soft);border:1px solid var(--line);border-radius:8px;padding:8px;white-space:pre-wrap"></pre>
          </details>
        </div>
      </div>
    </details>
    ...
    <script>
      (function () {
        ...
        function turnCard(role, text, ms) {
          const card = el('div', { style: 'padding:11px 14px;border-radius:10px;border:1px solid var(--line);' + (role === 'coach' ? 'background:var(--accent-tint);border-color:transparent' : 'background:var(--soft)') });
          const head = el('div', { style: 'font-size:10.5px;font-weight:600;text-transform:uppercase;letter-spacing:.06em;color:' + (role === 'coach' ? 'var(--accent)' : 'var(--muted)') + ';margin-bottom:4px' });
          head.textContent = role.toUpperCase() + (ms != null ? ' · ' + ms + 'ms' : '');
          const body = el('div', { style: 'font-size:14px;white-space:pre-wrap;color:var(--ink)' });
          body.textContent = text;
        ...
        convForm?.addEventListener('submit', async (e) => {
          ...
          const payload = {
            ...
            seedMessage: fd.get('seedMessage'),
          ...
          try {
            ...
            while (true) {
              ...
              while ((idx = buf.indexOf('\\n\\n')) !== -1) {
                ...
                if (evt.type === 'start') { convStatus.textContent = 'running · 0/' + evt.maxTurns; }
                else if (evt.type === 'user_turn') { convTranscript.appendChild(turnCard('user', evt.text)); convTranscript.lastElementChild.scrollIntoView({ behavior: 'smooth', block: 'end' }); }
                else if (evt.type === 'coach_turn') { coachTurns++; convTranscript.appendChild(turnCard('coach', evt.text, evt.ms)); convStatus.textContent = 'running · ' + coachTurns + ' coach turn(s) so far'; convTranscript.lastElementChild.scrollIntoView({ behavior: 'smooth', block: 'end' }); }
                else if (evt.type === 'done') { convStatus.textContent = 'done · ' + (evt.turns?.length || 0) + ' total turns'; }
                else if (evt.type === 'error') { convStatus.textContent = 'error: ' + evt.message; }
              }
            }
          } catch (err) {
            convStatus.textContent = 'network error: ' + err.message;
          } finally {
