/*
 * Coach Studio admin — Wix badge auth (12-Aug-2026; restored 17-Sep-2026
 * after a Codex commit reverted it to a shared ADMIN_PASSWORD form — see
 * git history around 21fdeb7 "ACD-13: implement custom Admin login form").
 *
 * Sign-in delegated to Wix Members: user is redirected to Wix's own login,
 * which returns an authorization code we exchange for a member identity.
 * Access controlled by a single Wix Members Badge — the operator adds/removes
 * people in the Wix dashboard, badge propagates in ≤60s.
 *
 * Env vars:
 *   WIX_CLIENT_ID       Wix Headless client id (public — sign-in only)
 *   WIX_CLIENT_SECRET   Wix Headless client secret (server-side badge read)
 *   SESSION_SECRET      HMAC key for signing the local session cookie
 *   PUBLIC_ORIGIN       (optional) https origin used to compute the callback
 *                       URI; falls back to RAILWAY_PUBLIC_DOMAIN or localhost.
 *
 * Cookie is `admin_session=<base64url(payload)>.<base64url(HMAC)>` with the
 * payload `{ sub: 'admin', m: memberId, e: email, n: name, iat, exp }`.
 * 4h TTL. Every /admin/* request re-checks the badge against Wix.
 */
...
const semanticExtrasStore = require('./semanticExtrasStore');
const wixOauth = require('./wixOauth');
...
function verifySession(cookieVal) {
    ...
    // Wix identity is mandatory. Sessions minted by the Sep-2026 password
    // detour carried `u` instead of `m` and must fail this shape check so
    // their holders bounce to the Wix sign-in rather than continuing under
    // a stale payload.
...
function makeSessionCookie({ memberId, email, name }) {
    if (!memberId) throw new Error('makeSessionCookie: memberId is required');
    ...
    // Secure only on https. HttpOnly keeps the session out of browser JS;
    // SameSite=Lax prevents cross-site POSTs from riding the cookie while
    // still surviving the top-level navigation back from Wix.
...
// Which build this is, shown on the sign-in screen before anybody signs in —
// ported from PIMS (src/lib/pims/build.ts): "a screenshot of a bug report
// carries the answer with it" instead of QA and engineering discovering
// after the fact that they were looking at two different deploys.
//
// `git rev-parse` fails on Railway: the runtime container has no .git (build
// and runtime are different filesystems here). Confirmed by loading this on
// staging and seeing "build dev" — a fixed string that looks like a normal
// local value instead of admitting the lookup failed, which is exactly the
// silent-degradation shape this feature exists to prevent in the first
// place. DEPLOY_TRIGGER is the fallback because it is already guaranteed to
// be set: every deploy of this service bumps it (see AFAZERES/CLAUDE.md —
// Railway redeploys are triggered by bumping this var, not by
// `railway up`), and it's already a unique, deploy-describing string.
// Read once at process start and cached — none of these sources change
// while the process runs.
let _buildLabel = null;
function buildLabel() {
    if (_buildLabel !== null) return _buildLabel;
    if (process.env.RAILWAY_GIT_COMMIT_SHA) {
        _buildLabel = process.env.RAILWAY_GIT_COMMIT_SHA.slice(0, 7);
        return _buildLabel;
    }
    try {
        const sha = execFileSync('git', ['rev-parse', '--short', 'HEAD'], { cwd: __dirname + '/..', encoding: 'utf8' }).trim();
        const iso = execFileSync('git', ['log', '-1', '--format=%cI'], { cwd: __dirname + '/..', encoding: 'utf8' }).trim();
        const at = new Date(iso);
        const when = Number.isNaN(at.getTime()) ? '' : at.toLocaleString('en-GB', {
            timeZone: 'UTC', day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit'
        }) + ' UTC';
        _buildLabel = when ? `${sha} · ${when}` : sha;
        return _buildLabel;
    } catch (_) { /* no .git here — fall through */ }
    if (process.env.DEPLOY_TRIGGER) {
        _buildLabel = process.env.DEPLOY_TRIGGER;
        return _buildLabel;
    }
    _buildLabel = 'unknown build';
    return _buildLabel;
}

// Coach Studio logo — flowing "C" with two convergence stars.
// Inlined as a data URI so there's no separate request + no CSP issue.
// %23 is URL-encoded "#" — safe inside an SVG data URI's utf8 payload.
const FAVICON_HREF = "data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 128 128'><defs><linearGradient id='g' x1='18' y1='104' x2='102' y2='20' gradientUnits='userSpaceOnUse'><stop offset='0' stop-color='%23087F79'/><stop offset='.55' stop-color='%2310A7B0'/><stop offset='1' stop-color='%2332C5E5'/></linearGradient></defs><path d='M91 27C82 20 71 16 59 16 32.5 16 11 37.5 11 64s21.5 48 48 48c12 0 23-4.4 31.5-11.7L76.8 84.7A27.2 27.2 0 0 1 59 91.3 27.3 27.3 0 1 1 76.6 43.8L91 27Z' fill='url(%23g)'/><path d='M101 43c1.8 9.6 4.4 12.2 14 14-9.6 1.8-12.2 4.4-14 14-1.8-9.6-4.4-12.2-14-14 9.6-1.8 12.2-4.4 14-14Z' fill='%2332C5E5'/><path d='M109 16c1.1 5.9 2.7 7.5 8.6 8.6-5.9 1.1-7.5 2.7-8.6 8.6-1.1-5.9-2.7-7.5-8.6-8.6 5.9-1.1 7.5-2.7 8.6-8.6Z' fill='%2310A7B0'/></svg>";

// The Google "G" mark — four flat paths, the standard public brand mark used
// on every "Sign in with Google" button. Inlined rather than fetched so it
// can't flash in after the button.
const GOOGLE_MARK_SVG = `<svg width="18" height="18" viewBox="0 0 48 48" aria-hidden="true" style="margin-right:10px;flex:none">
...
/*
 * Sign-in screen — structure ported from PIMS (src/routes/login.tsx,
 * 12-Sep-2026 shape), which is the actual reference Willian asked to be
 * mirrored. The redirect to Wix is not something either app avoids — PIMS's
 * own comment says it plainly: "The email and password form is gone, and its
 * absence is the point." What PIMS actually built instead of a bare
 * "Sign in with TT account" link is TWO real doors on a real page:
 *
 *   - "Continue with Google" — passes idp=google, so Wix skips its own
 *     dialog and goes straight to Google. Matters for anyone who has never
 *     been on the Wix site before.
 *   - "Continue with email" — the ordinary Wix-hosted Log In / Sign Up
 *     dialog (username or email + password, on Wix's side, never ours).
 *
 * No password field lives here, deliberately, for the same reason PIMS has
 * none: a password Coach Studio itself checked would need a reset flow, and
 * an admin-issued reset link is a privilege escalation wearing a different
 * hat (that's the exact finding that made PIMS remove its own password
 * form). Wix already owns credentials; re-implementing that here would be a
 * second, weaker copy of it.
 *
 * Both buttons POST to /auth/wix/begin (JSON) rather than being a plain
 * <a href>, so the click has a real, per-button "Taking you there…" state
 * before the actual cross-site navigation — the thing a bare link can't do.
 */
function loginPage({ error } = {}) {
    const configured = wixOauth.loginConfigured();
    return `<!doctype html>
<html><head>
<meta charset="utf-8">
<title>Coach Studio — Sign in</title>
<meta name="viewport" content="width=device-width,initial-scale=1">
<link rel="icon" type="image/svg+xml" href="${FAVICON_HREF}">
<style>
  * { box-sizing: border-box; }
  body { margin:0; font-family:-apple-system,BlinkMacSystemFont,"Inter",sans-serif;
    min-height:100vh; display:flex; flex-direction:column; background:#f4f7f9; color:#0b1a24; }

  /* Full-width bar, matching the approved Coach Studio mockup: brand mark on
     the left, a "Secure workspace" status pill on the right. Purely a trust
     signal — it does not gate anything the page below doesn't already. */
  .topbar { flex:none; height:56px; display:flex; align-items:center; justify-content:space-between;
    padding:0 28px; background:#071D26; border-bottom:1px solid rgba(255,255,255,.06); }
  .topbar .brand { display:flex; align-items:center; gap:10px; }
  .topbar .brand svg { width:26px; height:26px; }
  .topbar .brand span { font-weight:600; font-size:15px; color:#f0f6fa; letter-spacing:-.01em; }
  .securepill { display:inline-flex; align-items:center; gap:6px; font-size:12px; color:#a7f3d0;
    background:rgba(20,184,166,.14); border:1px solid rgba(20,184,166,.30); padding:4px 10px; border-radius:999px; }
  .securepill i { width:6px; height:6px; border-radius:50%; background:#4ade80; box-shadow:0 0 0 3px rgba(74,222,128,.22); }

  .body { flex:1; display:flex; }
  @media (min-width: 900px) { .body { flex-direction:row; } }
  @media (max-width: 899px) { .body { flex-direction:column; } }

  .hero { position:relative; overflow:hidden; flex:none;
    background:
      radial-gradient(circle at 15% 20%, rgba(50,197,229,.20), transparent 45%),
      linear-gradient(150deg, #0B2932 0%, #071D26 60%, #061920 100%);
    color:#dbe6eb; padding:44px 40px; display:flex; flex-direction:column; justify-content:space-between; }
  @media (min-width: 900px) { .hero { width:54%; min-height:calc(100vh - 56px); } }
  @media (max-width: 899px) { .hero { padding:30px 24px 36px; } }
  .hero::before { content:""; position:absolute; inset:0; pointer-events:none; opacity:.5;
    background-image:
      radial-gradient(circle, rgba(50,197,229,.28) 1px, transparent 1.3px),
      linear-gradient(0deg, transparent 0 55px, rgba(50,197,229,.05) 55px 56px),
      linear-gradient(90deg, transparent 0 55px, rgba(50,197,229,.05) 55px 56px);
    background-size:56px 56px; mask-image:radial-gradient(ellipse 100% 80% at 15% 15%, black 0%, transparent 75%); }
  .hero > * { position:relative; z-index:1; }

  .kicker { display:flex; align-items:center; gap:8px; font:600 10.5px/1 inherit; text-transform:uppercase;
    letter-spacing:.14em; color:#5eead4; }
  .kicker::before { content:""; width:18px; height:1px; background:#5eead4; }

  .hero-copy { margin-top:18px; }
  .hero-copy h1 { font-size:28px; line-height:1.28; margin:0 0 12px; font-weight:700; color:#f0f6fa; letter-spacing:-.015em; }
  @media (min-width: 900px) { .hero-copy h1 { font-size:36px; } }
  .hero-copy h1 .accent { background:linear-gradient(90deg,#5eead4,#38bdf8); -webkit-background-clip:text; background-clip:text; color:transparent; }
  .hero-copy p { font-size:14.5px; line-height:1.65; color:#8fa6b1; margin:0; max-width:42ch; }

  .chiprow { display:flex; flex-wrap:wrap; gap:8px; margin-top:20px; }
  .chip { display:inline-flex; align-items:center; gap:7px; font-size:12.5px; color:#dbe6eb;
    background:rgba(255,255,255,.045); border:1px solid rgba(255,255,255,.10); padding:6px 12px; border-radius:999px; }
  .chip svg { width:13px; height:13px; color:#5eead4; flex:none; }

  /* Decorative only — desktop-width real estate, hidden on narrow viewports
     rather than squeezed, matching how the rest of Coach Studio drops
     ornament before it drops legibility. */
  .floatcards { display:none; }
  @media (min-width: 1080px) {
    .floatcards { display:block; position:relative; height:280px; margin-top:24px; }
    .fcard { position:absolute; width:200px; background:rgba(11,41,50,.75); backdrop-filter:blur(6px);
      border:1px solid rgba(255,255,255,.08); border-radius:12px; padding:12px 14px; box-shadow:0 12px 30px -10px rgba(0,0,0,.5); }
    .fcard .t { font-size:11px; color:#8fa6b1; margin-bottom:8px; }
    .fcard .bar { height:5px; border-radius:3px; background:rgba(255,255,255,.08); margin:5px 0; overflow:hidden; }
    .fcard .bar i { display:block; height:100%; border-radius:3px; }
    .fc1 { top:0; left:30px; }
    .fc1 .bar:nth-child(2) i { width:78%; background:linear-gradient(90deg,#2dd4bf,#38bdf8); }
    .fc1 .bar:nth-child(3) i { width:45%; background:rgba(255,255,255,.18); }
    .fc1 .bar:nth-child(4) i { width:60%; background:rgba(255,255,255,.18); }
    .fc2 { top:95px; left:150px; }
    .fc2 .bar:nth-child(2) i { width:88%; background:linear-gradient(90deg,#5eead4,#2dd4bf); }
    .fc2 .bar:nth-child(3) i { width:55%; background:rgba(255,255,255,.18); }
    .fc3 { top:195px; left:55px; }
    .fc3 .chart { display:flex; align-items:flex-end; gap:3px; height:34px; }
    .fc3 .chart i { flex:1; border-radius:2px 2px 0 0; background:linear-gradient(180deg,#38bdf8,#2563eb); }
  }

  .footkicker { font:600 10px/1 inherit; text-transform:uppercase; letter-spacing:.14em; color:#5a7480; }

  .panel { flex:1; display:flex; align-items:center; justify-content:center; padding:32px 24px; }
  .card { width:100%; max-width:360px; }
  .card h2 { font-size:20px; margin:0 0 4px; font-weight:600; }
  .card .sub { font-size:13px; color:#7c8a97; margin:0 0 22px; }

  .doorbtn { width:100%; display:flex; align-items:center; justify-content:center;
    font:inherit; font-size:14px; font-weight:600; padding:11px 16px; border-radius:10px;
    cursor:pointer; transition:filter .15s ease, transform .1s ease; }
  .doorbtn:active { transform:translateY(1px); }
  .doorbtn:disabled { opacity:.7; cursor:default; }
  .doorbtn.outline { background:#fff; border:1px solid #dbe0e6; color:#0b1a24; }
  .doorbtn.outline:hover:not(:disabled) { border-color:#b8c2ca; }
  .doorbtn.primary { background:linear-gradient(135deg,#0d9488 0%,#3987e5 100%); border:none; color:#fff;
    box-shadow:0 3px 10px -2px rgba(13,148,136,.35); margin-top:12px; }
  .doorbtn.primary:hover:not(:disabled) { filter:brightness(1.05); }

  .divider { display:flex; align-items:center; gap:10px; margin:14px 0; }
  .divider span:not(.txt) { height:1px; flex:1; background:#e2e6ea; }
  .divider .txt { font-size:10.5px; text-transform:uppercase; letter-spacing:.08em; color:#9aa7b0; }

  .spinner { width:14px; height:14px; margin-right:8px; border:2px solid rgba(0,0,0,.15);
    border-top-color:currentColor; border-radius:50%; animation:spin .7s linear infinite; flex:none; }
  .doorbtn.primary .spinner { border-color:rgba(255,255,255,.35); border-top-color:#fff; }
  @keyframes spin { to { transform:rotate(360deg); } }

  .err { font-size:12.5px; color:#c23b3b; margin-top:12px; }
  .help { font-size:12px; line-height:1.6; color:#7c8a97; margin-top:18px; max-width:42ch; }
  .help + .help { margin-top:8px; }
  .lockline { display:flex; align-items:center; gap:6px; font-size:11.5px; color:#9aa7b0; margin-top:20px; }
  .buildline { font-size:10px; color:#c1c9d0; font-family:ui-monospace,Menlo,Consolas,monospace; margin-top:6px; }
</style>
</head>
<body>
  <div class="topbar">
    <div class="brand">
      <svg viewBox="0 0 128 128" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
        <defs><linearGradient id="tlg" x1="18" y1="104" x2="102" y2="20" gradientUnits="userSpaceOnUse">
          <stop offset="0" stop-color="#087F79"/><stop offset=".55" stop-color="#10A7B0"/><stop offset="1" stop-color="#32C5E5"/>
        </linearGradient></defs>
        <path d="M91 27C82 20 71 16 59 16 32.5 16 11 37.5 11 64s21.5 48 48 48c12 0 23-4.4 31.5-11.7L76.8 84.7A27.2 27.2 0 0 1 59 91.3 27.3 27.3 0 1 1 76.6 43.8L91 27Z" fill="url(#tlg)"/>
        <path d="M101 43c1.8 9.6 4.4 12.2 14 14-9.6 1.8-12.2 4.4-14 14-1.8-9.6-4.4-12.2-14-14 9.6-1.8 12.2-4.4 14-14Z" fill="#32C5E5"/>
        <path d="M109 16c1.1 5.9 2.7 7.5 8.6 8.6-5.9 1.1-7.5 2.7-8.6 8.6-1.1-5.9-2.7-7.5-8.6-8.6 5.9-1.1 7.5-2.7 8.6-8.6Z" fill="#10A7B0"/>
      </svg>
      <span>Coach Studio</span>
    </div>
    <div class="securepill"><i></i>Secure workspace</div>
  </div>

  <div class="body">
  <div class="hero">
    <div>
      <div class="kicker">AI coaching infrastructure</div>
      <div class="hero-copy">
        <h1>Design, test, and <span class="accent">observe your AI coach.</span></h1>
        <p>Manage personas, inspect prompt composition, simulate real conversations, and monitor production — all in one workspace.</p>
      </div>
      <div class="chiprow">
        <span class="chip"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 6h16M4 12h10M4 18h7"/></svg>Prompt intelligence</span>
        <span class="chip"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="6 3 20 12 6 21 6 3"/></svg>Live simulation</span>
        <span class="chip"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 3v18h18M7 15l4-6 3 3 5-8"/></svg>Production observability</span>
      </div>
      <div class="floatcards">
        <div class="fcard fc1"><div class="t">Prompt composition</div>
          <div class="bar"></div><div class="bar"></div><div class="bar"></div>
        </div>
        <div class="fcard fc2"><div class="t">Live simulation</div>
          <div class="bar"></div><div class="bar"></div>
        </div>
        <div class="fcard fc3"><div class="t">Observability</div>
          <div class="chart"><i style="height:35%"></i><i style="height:60%"></i><i style="height:40%"></i><i style="height:80%"></i><i style="height:55%"></i><i style="height:70%"></i><i style="height:45%"></i></div>
        </div>
      </div>
    </div>
    <div class="footkicker">Build better coaching experiences</div>
  </div>

  <div class="panel">
    <div class="card">
      <h2>Sign in</h2>
      <p class="sub">Coach Studio uses your Talent Transformation account.</p>

      ${configured ? `
      <button type="button" class="doorbtn outline" id="btnGoogle" onclick="startWix('google')">
        ${GOOGLE_MARK_SVG}<span>Continue with Google</span>
      </button>

      <div class="divider"><span></span><span class="txt">or</span><span></span></div>

      <button type="button" class="doorbtn primary" id="btnEmail" onclick="startWix()">
        <span>Continue with email</span>
      </button>

      <p id="err" class="err" ${error ? '' : 'hidden'}>${escapeHtml(error || '')}</p>

      <p class="help">First time here? Continuing with either option creates or matches your Talent Transformation account — nothing to sign up for separately.</p>
      <p class="help">If you can't sign in, ask Willian to grant your account access.</p>
      ` : `
      <p class="err" style="margin-top:4px">Sign-in is not configured on this deployment.</p>
      `}

      <div class="lockline">🔒 Authorized team members only</div>
      <div class="buildline">build ${escapeHtml(buildLabel())}</div>
    </div>
  </div>
  </div>
  ...
  <script>
    var busy = false;
    function setPending(which) {
      var g = document.getElementById('btnGoogle'), e = document.getElementById('btnEmail');
      if (!g || !e) return;
      g.disabled = busy; e.disabled = busy;
      g.innerHTML = (which === 'google' && busy)
        ? '<span class="spinner"></span><span>Taking you there…</span>'
        : ${JSON.stringify(GOOGLE_MARK_SVG)} + '<span>Continue with Google</span>';
      e.innerHTML = (which === 'email' && busy)
        ? '<span class="spinner"></span><span>Taking you there…</span>'
        : '<span>Continue with email</span>';
    }
    async function startWix(provider) {
      if (busy) return;
      busy = true;
      ...
      try {
        var res = await fetch('/auth/wix/begin', {
          method: 'POST',
        ...
        if (!data.ok) {
          if (err) { err.textContent = data.message || 'Could not start signing in.'; err.hidden = false; }
          busy = false; setPending(null);
          return;
        }
        // Full navigation, not fetch-follow: the destination is Wix.
        window.location.href = data.url;
      } catch (e) {
        if (err) { err.textContent = 'Could not start signing in through Talent Transformation.'; err.hidden = false; }
        busy = false; setPending(null);
      }
    }
  </script>
...
// Shown when the sign-in worked but the account lacks the Coach Studio Admin
// badge (or when Wix could not tell us either way). Kept plain HTML rather
// than shellChrome() because the shell itself is behind the gate this page
// explains.
function denialPage({ email, reason, failed }) {
    const title = failed ? 'Sign-in check failed' : 'Access not granted';
    return `<!doctype html>
<html><head>
<meta charset="utf-8">
<title>Coach Studio — ${escapeHtml(title)}</title>
<meta name="viewport" content="width=device-width,initial-scale=1">
<link rel="icon" type="image/svg+xml" href="${FAVICON_HREF}">
<script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="min-h-screen bg-gray-50 flex items-center justify-center p-4">
  <div class="w-full max-w-md bg-white rounded-2xl shadow-sm p-6 space-y-4 border border-gray-200">
    <div>
      <div class="text-lg font-semibold text-gray-900">${escapeHtml(title)}</div>
      ${email ? `<div class="text-xs text-gray-500 mt-1">Signed in as <span class="font-mono">${escapeHtml(email)}</span></div>` : ''}
    </div>
    <p class="text-sm text-gray-700">${escapeHtml(reason || 'No further detail is available.')}</p>
    ${failed
        ? '<p class="text-xs text-gray-500">This is not necessarily a permissions problem — sign-in may be temporarily unavailable. Try again in a minute.</p>'
        : '<p class="text-xs text-gray-500">Ask Willian to grant you Coach Studio access. It takes effect within a minute of being added.</p>'}
    <form method="POST" action="/auth/logout" class="pt-2">
      <button type="submit" class="text-sm text-teal-700 hover:text-teal-900 underline">Sign out and try a different account</button>
    </form>
  </div>
</body></html>`;
}
...
<style>
  ...
  :root { color-scheme: light dark;
    --bg:#f4f7f9; --card:#ffffff; --soft:#eef1f4;
    --ink:#0b1a24; --ink2:#354b5c; --muted:#7c8a97;
    --line:#dbe0e6; --axis:#c2ccd4; --ring:rgba(7,29,38,.08); --ring-hi:rgba(7,29,38,.14);
    /* Navy tones — reserved for the appbar and agent identity strip. */
    --navy:#071D26; --navy-mid:#0B2932; --navy-hi:#0f3541;
    --navy-ink:#dbe6eb;   /* text on navy */
    --navy-muted:#8fa6b1;
    --s1:#0d9488; --s2:#eb6834; --s3:#3987e5;
    --accent:#0d9488; --accent-hi:#14b8a6; --accent-tint:rgba(13,148,136,.10);
    --agent:#2b7bd7;      /* actions produced by the agent */
    --agent-tint:rgba(43,123,215,.10);
    ...
    --appbar-bg:
      radial-gradient(circle at 12% 0%, rgba(17, 156, 149, .22), transparent 34%),
      linear-gradient(110deg, #071D26 0%, #0B2932 55%, #071A23 100%);
  }
  @media (prefers-color-scheme: dark) { :root {
    --bg:#0b1015; --card:#141a20; --soft:#1a212a;
    --ink:#e6ecf2; --ink2:#b4c0cb; --muted:#7e8a95;
    --line:#252d36; --axis:#333d47; --ring:rgba(255,255,255,.06); --ring-hi:rgba(255,255,255,.12);
    --navy:#050f14; --navy-mid:#081a22; --navy-hi:#0d2632;
    --navy-ink:#dbe6eb; --navy-muted:#7e93a0;
    --s1:#14b8a6; --s2:#d95926; --s3:#5b9be8;
    --accent:#14b8a6; --accent-hi:#2dd4bf; --accent-tint:rgba(20,184,166,.14);
    --agent:#5aa4ff; --agent-tint:rgba(90,164,255,.14);
    --good:#22c55e; --good-text:#22c55e; --warn:#fab219; --crit:#f87171;
    --appbar-bg:
      radial-gradient(circle at 12% 0%, rgba(17, 156, 149, .22), transparent 34%),
      linear-gradient(110deg, #050f14 0%, #081a22 55%, #030b0f 100%);
  } }
  * { box-sizing: border-box; }
  ...
  .layout { display:grid; grid-template-columns: minmax(340px, 34%) 1fr; flex:1; min-height:0; position:relative; }
  ...
  /* -- appbar (global topbar) -- */
  .appbar { height:56px; flex:none; background:var(--appbar-bg); color:var(--navy-ink);
    display:flex; align-items:center; padding:0 22px; gap:22px;
    border-bottom:1px solid rgba(255,255,255,.06);
    box-shadow: 0 1px 0 rgba(0,0,0,.35);
    position:relative; z-index:15; }
  .appbar .brand-mark { display:flex; align-items:center; gap:10px; flex:none; }
  .appbar .brand-mark svg { width:34px; height:34px; display:block;
    filter: drop-shadow(0 2px 8px rgba(50, 197, 229, .35)); }
  .appbar .brand-mark .brand-word { font: 600 15.5px/1 "Inter",-apple-system,sans-serif;
    letter-spacing:-.01em; color:#f4f7fa; }
  .appbar nav.appbar-nav { display:flex; gap:2px; margin-left:26px; font-size:13.5px; align-items:center; flex:1;
    justify-content:center; }
  .appbar nav.appbar-nav a { color:var(--navy-muted); padding:8px 14px; border-radius:8px;
    text-decoration:none; font-weight:500; transition:all .12s ease; }
  .appbar nav.appbar-nav a:hover { color:#f4f7fa; background:rgba(255,255,255,.06); text-decoration:none; }
  .appbar nav.appbar-nav a.active { color:#f4f7fa; background:rgba(20,184,166,.22);
    box-shadow: inset 0 0 0 1px rgba(20,184,166,.4); }
  .appbar nav.appbar-nav .nav-sep { color:rgba(255,255,255,.14); }
  .appbar nav.appbar-nav .nav-group { position:relative; color:var(--navy-muted); padding:8px 14px;
    border-radius:8px; font-weight:500; cursor:default; user-select:none; }
  .appbar nav.appbar-nav .nav-group:hover, .appbar nav.appbar-nav .nav-group:focus-within {
    color:#f4f7fa; background:rgba(255,255,255,.06); }
  .appbar nav.appbar-nav .nav-group.active-group { color:#f4f7fa; background:rgba(20,184,166,.22);
    box-shadow: inset 0 0 0 1px rgba(20,184,166,.4); }
  .appbar nav.appbar-nav .nav-group .nav-menu { display:none; position:absolute; top:calc(100% + 4px);
    left:0; z-index:20; background:var(--navy-mid); border:1px solid rgba(255,255,255,.08);
    border-radius:10px; box-shadow:0 12px 32px -6px rgba(0,0,0,.5); padding:6px; min-width:170px; }
  .appbar nav.appbar-nav .nav-group:hover .nav-menu,
  .appbar nav.appbar-nav .nav-group:focus-within .nav-menu { display:flex; flex-direction:column; }
  .appbar nav.appbar-nav .nav-menu a { padding:7px 12px; font-size:13px; color:var(--navy-muted);
    border-radius:6px; }
  .appbar nav.appbar-nav .nav-menu a:hover { color:#f4f7fa; background:rgba(255,255,255,.08); }
  .appbar .app-right { display:flex; align-items:center; gap:14px; flex:none; }
  .appbar .app-status { display:inline-flex; align-items:center; gap:7px; font-size:12.5px;
    color:var(--navy-muted); }
  .appbar .app-status::before { content:""; width:8px; height:8px; border-radius:50%;
    background:#4ade80; box-shadow:0 0 0 3px rgba(74,222,128,.25); }
  .appbar .app-user { display:inline-flex; align-items:center; gap:8px; font-size:13px; color:var(--navy-ink);
    padding:5px 10px; border-radius:8px; }
  .appbar .app-user form { margin:0; }
  .appbar .app-user button { background:transparent; border:0; color:var(--navy-muted); font:inherit;
    padding:0; cursor:pointer; font-size:12.5px; }
  .appbar .app-user button:hover { color:#f4f7fa; text-decoration:underline; }

  /* -- agent panel -- */
  .agente { display:flex; flex-direction:column; border-right:1px solid var(--line);
    background: var(--card);
    min-height:0; position:relative; }
  /* Single scroll container: identity block scrolls WITH the work stream so
     it doesn't steal prime real estate. A slim sticky mini-strip surfaces
     when the full identity has scrolled out of view — click it to jump back. */
  .agent-scroll { flex:1; overflow-y:auto; min-height:0; position:relative;
    scrollbar-width:thin; scrollbar-color: var(--line) transparent; }
  .agent-scroll::-webkit-scrollbar { width:8px; }
  .agent-scroll::-webkit-scrollbar-thumb { background: var(--line); border-radius:4px; }
  ...
  .agent-id-mini.visible { height:40px; opacity:1; pointer-events:auto; }
  .agent-id-mini::before {
    content:""; width:7px; height:7px; border-radius:50%;
    background:#4ade80; box-shadow:0 0 0 3px rgba(74,222,128,.22); flex:none;
  }
  .agent-id-mini .mini-name { font-weight:600; color:#f0f6fa; }
  .agent-id-mini .mini-kicker { font: 600 9.5px/1 -apple-system,"Inter",sans-serif;
    text-transform:uppercase; letter-spacing:.14em; color:rgba(94, 234, 212, .85); margin-left:auto; }

  /* Blueprint-navy backdrop: soft radial glow upper-left, faint teal grid
     with dots at intersections, and two whispered diagonal scratch strokes.
     Pure CSS reproduction of Willian's reference image, scoped to just the
     identity header at the top of the coach panel. */
  .agente header.agent-id {
    background:
      linear-gradient(115deg, transparent 0 44.9%, rgba(50,197,229,.05) 44.95%, rgba(50,197,229,.05) 45.25%, transparent 45.3%),
      linear-gradient(122deg, transparent 0 67.9%, rgba(50,197,229,.04) 67.95%, rgba(50,197,229,.04) 68.25%, transparent 68.3%),
      radial-gradient(460px 340px at 22% 55%, rgba(50, 197, 229, 0.18), transparent 65%),
      linear-gradient(180deg, #06171f 0%, #071D26 55%, #0a2635 100%);
  ...
  .agente header.agent-id::after {
    content:""; position:absolute; inset:0; pointer-events:none;
    background-image:
      radial-gradient(circle, rgba(50,197,229,.32) 1.1px, transparent 1.4px),
      linear-gradient(0deg, transparent 0 47px, rgba(50,197,229,.06) 47px 48px),
      linear-gradient(90deg, transparent 0 47px, rgba(50,197,229,.06) 47px 48px);
    background-size: 48px 48px, 48px 48px, 48px 48px;
    background-position: 0 0;
    mix-blend-mode: screen;
    opacity:.55;
    mask-image: radial-gradient(ellipse 110% 100% at 30% 55%, black 0%, rgba(0,0,0,.75) 60%, transparent 100%);
    -webkit-mask-image: radial-gradient(ellipse 110% 100% at 30% 55%, black 0%, rgba(0,0,0,.75) 60%, transparent 100%);
  }
  .agente header.agent-id > * { position:relative; z-index:1; }
  .agente header.agent-id > * { position:relative; z-index:1; }
  .agent-id .kicker {
    font: 600 10.5px/1 -apple-system,"Inter",sans-serif; text-transform:uppercase;
    letter-spacing:.14em; color:rgba(94, 234, 212, .85); display:inline-flex; align-items:center; gap:6px;
  }
  .agent-id .kicker::before { content:""; width:6px; height:6px; border-radius:50%;
    background:#4ade80; box-shadow:0 0 0 3px rgba(74,222,128,.28); }
  .agent-id h1 { margin:0; font: 600 17px/1.25 "Inter",sans-serif; color:#f0f6fa;
    letter-spacing:-.01em; }
  .agent-id .scope { font-size:12.5px; color:var(--navy-muted); line-height:1.4; margin:0; }
  .agent-id .agent-chips { display:flex; flex-wrap:wrap; gap:6px; margin-top:2px; }
  .agent-id .agent-chip { font: 500 11px/1 inherit; padding:4px 9px; border-radius:999px;
    background:rgba(255,255,255,.06); border:1px solid rgba(255,255,255,.08);
    color:var(--navy-muted); }
  .agent-id .agent-chip.ok { color:#a7f3d0; background:rgba(20,184,166,.14); border-color:rgba(20,184,166,.35); }
  .status-dot { display:none; }   /* replaced by kicker dot */
  /* .chat lives inside .agent-scroll now — it just flows; the wrapper scrolls. */
  .chat { padding:16px 18px; display:flex; flex-direction:column; gap:14px; min-height:240px; }
  ...
  .topo h2 { font-size:20px; margin:0; font-weight:600; letter-spacing:-.015em; }
  .topo .meta { font-size:13px; color:var(--muted); }
...
<div class="app">
<header class="appbar">
  <div class="brand-mark" aria-label="Coach Studio">
    <svg viewBox="0 0 128 128" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      <defs>
        <linearGradient id="cslogo-flow" x1="18" y1="104" x2="102" y2="20" gradientUnits="userSpaceOnUse">
          <stop offset="0" stop-color="#087F79"/>
          <stop offset="0.55" stop-color="#10A7B0"/>
          <stop offset="1" stop-color="#32C5E5"/>
        </linearGradient>
      </defs>
      <path d="M91 27C82 20 71 16 59 16 32.5 16 11 37.5 11 64s21.5 48 48 48c12 0 23-4.4 31.5-11.7L76.8 84.7A27.2 27.2 0 0 1 59 91.3 27.3 27.3 0 1 1 76.6 43.8L91 27Z" fill="url(#cslogo-flow)"/>
      <path d="M79.5 48.5A21.8 21.8 0 0 0 64 42v15.5c2.5 0 4.8.8 6.7 2.2l8.8-11.2Z" fill="#071D26" opacity=".16"/>
      <path d="M101 43c1.8 9.6 4.4 12.2 14 14-9.6 1.8-12.2 4.4-14 14-1.8-9.6-4.4-12.2-14-14 9.6-1.8 12.2-4.4 14-14Z" fill="#32C5E5"/>
      <path d="M109 16c1.1 5.9 2.7 7.5 8.6 8.6-5.9 1.1-7.5 2.7-8.6 8.6-1.1-5.9-2.7-7.5-8.6-8.6 5.9-1.1 7.5-2.7 8.6-8.6Z" fill="#10A7B0"/>
    </svg>
    <span class="brand-word">Coach Studio</span>
  </div>
  <nav class="appbar-nav" data-title="${escapeHtml(title)}">
...
<div class="layout">
  <section class="agente">
    <div class="agent-scroll" id="agentScroll">
      <div class="agent-id-mini" id="agentIdMini" title="Scroll up to expand" role="button" tabindex="0">
        <span class="mini-name">Production observer</span>
        <span class="mini-kicker">Active</span>
      </div>
      <header class="agent-id" id="agentIdFull">
        <span class="kicker">Active agent</span>
        <h1>Production observer</h1>
        <p class="scope">Monitoring Erica across sessions, prompts and tools.</p>
        <div class="agent-chips">
          <span class="agent-chip ok">Observing production</span>
          <span class="agent-chip">Reads logs and acts across the system</span>
        </div>
      </header>
    ...
    <div class="composer">
      <textarea id="composerText" rows="1" placeholder="Give the agent a goal…"></textarea>
      <button class="icone" id="composerClear" title="Clear work stream">✕</button>
  ...
  <section class="relatorio">
    <div class="topo" data-title="${escapeHtml(title)}">
      <h2 class="page-h">${escapeHtml(title || '')}</h2>
...
<script>
  (function () {
    ...
    function markActiveNav(title) {
      ...
      // Update the contextual page title in the right panel too.
      const pageH = document.querySelector('.topo .page-h');
      if (pageH) pageH.textContent = title || '';
    ...
    try { markActiveNav(document.querySelector('.appbar-nav')?.dataset.title || ''); } catch (_) {}

    // ---- Agent identity mini-strip ----
    // The full identity block scrolls with the work stream so it doesn't
    // dominate the panel. When the full block scrolls out of view we surface
    // a slim navy strip pinned at the top of the scroll wrapper: "●
    // Production observer · Active". Click it to jump back to the top.
    (function initAgentMini() {
      const wrap = document.getElementById('agentScroll');
      const full = document.getElementById('agentIdFull');
      const mini = document.getElementById('agentIdMini');
      if (!wrap || !full || !mini || !('IntersectionObserver' in window)) return;
      ...
      mini.addEventListener('click', jump);
    ...
    // Chat lives inside .agent-scroll — that wrapper owns overflow now, so
    // scrollBottom drives the wrapper. Falling back to chat itself keeps
    // pages that missed the refactor working.
    function scrollBottom() {
      const scroller = document.getElementById('agentScroll') || chat;
...
function sessionsPage({ items, tester, limit, bookmarkFilter, bookmarks }) {
    ...
    const rows = shown.length ? shown.map((s) => {
      const label = _semanticSessionLabel(s);
      const actor = s.actor?.email || s.actor?.userId
        || (s.actor?.objectId ? 'guest ' + s.actor.objectId.slice(0, 6) : 'guest');
      return `
      <tr class="bk-row">
        <td class="bk-session-cell">
          <a class="bk-label" href="/admin/sessions/${encodeURIComponent(s.sessionId)}">${escapeHtml(label)}</a>
          <div class="bk-sub">${escapeHtml(actor)}${s.actor?.tester ? ' · tester' : ''} · ${s.turns} turn${s.turns === 1 ? '' : 's'}</div>
        </td>
        <td class="px-3 py-2 text-xs" style="white-space:nowrap">${escapeHtml(_formatSessionTime(s.startedAt))}</td>
        <td class="px-3 py-2 text-xs" style="white-space:nowrap">${escapeHtml(_formatSessionTime(s.lastAt))}</td>
        ${bookmarkCell(s)}
      </tr>
      `;
    }).join('') : `<tr><td colspan="4" class="px-3 py-8 text-sm text-gray-500 text-center">No sessions match.</td></tr>`;
    ...
    <style>
      ...
      .bk-session-cell { padding:10px 12px; min-width:280px; max-width:420px; }
      .bk-label { display:block; font-size:13.5px; color:var(--ink); text-decoration:none;
        line-height:1.4; overflow:hidden; text-overflow:ellipsis; }
      .bk-label:hover { color:var(--accent); text-decoration:underline; }
    ...
    <section class="bg-white rounded-2xl border border-gray-200 shadow-sm" style="border:1px solid var(--line);border-radius:14px;background:var(--card);overflow:hidden">
      ...
      <table class="bk-table">
        <thead>
          <tr>
            <th style="min-width:280px">Session · first message</th>
            <th>Started</th>
            <th>Last</th>
            <th>Bookmark · note</th>
          </tr>
...
${escapeHtml(typeof e.result === 'string' ? e.result : JSON.stringify(e.result, null, 2))}</pre></details></div></div>`;
    ...
    // Semantic label: first non-empty user turn text — this is what humans
    // remember about a session ("the one where they asked about the offer").
    // Falls back to the persona name if user text was redacted.
    const firstUserTurn = data.entries.find((e) => e.type === 'turn' && e.role === 'user'
        && e.text && (typeof e.text === 'string' ? e.text.trim() : true));
    let semanticLabel = '';
    if (firstUserTurn) {
        if (typeof firstUserTurn.text === 'string') {
            semanticLabel = firstUserTurn.text.slice(0, 80);
        } else if (firstUserTurn.text && firstUserTurn.text.redacted) {
            semanticLabel = '(redacted user turn — ' + firstUserTurn.text.length + ' chars)';
        }
    }
    if (!semanticLabel) semanticLabel = '(no user text yet)';

    const bk = sessionBookmarks.get(sessionId) || {};
    const bookmarkBar = `
      <form method="POST" action="/admin/sessions/${encodeURIComponent(sessionId)}/bookmark" class="sd-bk-form">
        <input type="hidden" name="return" value="${escapeHtml('/admin/sessions/' + sessionId)}">
        <button type="submit" name="kind" value="${bk.kind === 'exemplar' ? '' : 'exemplar'}"
          class="sd-bk-btn ${bk.kind === 'exemplar' ? 'active exemplar' : ''}"
          title="${bk.kind === 'exemplar' ? 'Clear exemplar mark' : 'Mark as exemplar'}">⭐ Exemplar</button>
        <button type="submit" name="kind" value="${bk.kind === 'problem' ? '' : 'problem'}"
          class="sd-bk-btn ${bk.kind === 'problem' ? 'active problem' : ''}"
          title="${bk.kind === 'problem' ? 'Clear problem mark' : 'Mark as problem'}">⚠️ Problem</button>
        <input type="text" name="note" value="${escapeHtml(bk.note || '')}" maxlength="200"
          placeholder="add a short review note…"
          class="sd-bk-note ${bk.kind ? 'has-kind' : ''}"
          onchange="this.form.submit()"
          onblur="if (this.dataset.orig !== this.value) this.form.submit()"
          data-orig="${escapeHtml(bk.note || '')}">
      </form>`;

    const startedHuman = _formatSessionTime(start.t);
    const actorLabel = start.actor?.email || start.actor?.userId
        || (start.actor?.objectId ? 'guest ' + start.actor.objectId.slice(0, 8) + '…' : 'anonymous guest');

    const meta = `
    <style>
      .sd-hero { background:var(--card); border:1px solid var(--line); border-radius:14px; padding:18px 22px;
        display:flex; flex-direction:column; gap:10px; box-shadow:0 1px 3px var(--ring); }
      .sd-label { font-size:16px; font-weight:600; color:var(--ink); line-height:1.4; }
      .sd-meta-row { display:flex; flex-wrap:wrap; gap:12px 20px; font-size:12.5px; color:var(--muted); }
      .sd-meta-row b { color:var(--ink2); font-weight:600; }
      .sd-bk-form { display:flex; align-items:center; gap:8px; margin-top:2px; }
      .sd-bk-btn { background:var(--card); border:1px solid var(--line); padding:5px 12px; font-size:12.5px;
        border-radius:8px; cursor:pointer; color:var(--ink2); }
      .sd-bk-btn:hover { border-color:var(--accent); }
      .sd-bk-btn.active.exemplar { background:rgba(20, 184, 166, 0.12); color:var(--accent); border-color:transparent; font-weight:600; }
      .sd-bk-btn.active.problem { background:rgba(180, 83, 9, 0.14); color:var(--warn); border-color:transparent; font-weight:600; }
      .sd-bk-note { flex:1; min-width:220px; padding:6px 10px; font-size:12.5px; border:1px solid var(--line);
        border-radius:8px; background:var(--soft); color:var(--ink); }
      .sd-bk-note:focus { border-color:var(--accent); outline:none; background:var(--card); }
      .sd-sid { font-family:ui-monospace,Consolas,monospace; font-size:11px; color:var(--muted); }
    </style>
    <section class="sd-hero">
      <div class="sd-label">${escapeHtml(semanticLabel)}</div>
      <div class="sd-meta-row">
        <span><b>Actor</b> ${escapeHtml(actorLabel)} ${start.actor?.tester ? '<span style="color:var(--warn);font-size:11px">· tester</span>' : ''}</span>
        <span><b>Started</b> ${escapeHtml(startedHuman)}</span>
        <span><b>Entries</b> ${data.entries.length}${promptRefs.size ? ' · ' + promptRefs.size + ' prompt snapshot' + (promptRefs.size > 1 ? 's' : '') : ''}</span>
        <span class="sd-sid">${escapeHtml(sessionId)}</span>
      </div>
      ${bookmarkBar}
    </section>`;
    return shellChrome('Session · ' + semanticLabel.slice(0, 40), meta + '<section class="space-y-2" style="margin-top:14px">' + rows + '</section>');
}

// Human-friendly session time formatter used in the detail header, home,
// and the sessions list. Returns strings like "today 14:32", "yesterday 09:07",
// "3 days ago", or "Aug 3, 14:32" for older entries.
function _formatSessionTime(isoString) {
    if (!isoString) return '—';
    const d = new Date(isoString);
    if (isNaN(d.getTime())) return isoString;
    const now = new Date();
    const dayMs = 24 * 60 * 60 * 1000;
    const startOfDay = (x) => Date.UTC(x.getUTCFullYear(), x.getUTCMonth(), x.getUTCDate());
    ...
    if (diffDays > 1 && diffDays <= 7) return diffDays + ' days ago ' + hm;
    // Older: "Aug 3, 14:32"
    const months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
    return months[d.getUTCMonth()] + ' ' + d.getUTCDate() + ', ' + hm;
}

// Semantic label extractor for a session index row (uses only what listSessions
// exposes — falls back to a full read only if the index has no user turn yet).
// Cached per session in-memory to avoid re-reading NDJSON for every home render.
const _semanticLabelCache = new Map();
function _semanticSessionLabel(indexRow) {
    if (!indexRow || !indexRow.sessionId) return '(unknown session)';
    if (_semanticLabelCache.has(indexRow.sessionId)) return _semanticLabelCache.get(indexRow.sessionId);
    let label = '';
    try {
        const sess = sessionLog.readSession(indexRow.sessionId);
        if (sess && sess.entries) {
            const t = sess.entries.find((e) => e.type === 'turn' && e.role === 'user'
                && e.text && (typeof e.text === 'string' ? e.text.trim() : true));
            if (t) {
                if (typeof t.text === 'string') label = t.text.slice(0, 60);
                else if (t.text && t.text.redacted) label = '(redacted · ' + t.text.length + ' chars)';
            }
        }
    } catch (_) { /* absent = fall through */ }
    if (!label) label = indexRow.turns > 0 ? '(no user text visible)' : '(empty session)';
    _semanticLabelCache.set(indexRow.sessionId, label);
    return label;
}
...
function semanticStorePage() {
    ...
    const companyDocs = semanticExtrasStore.listDocs('company');
    const siteDocs = semanticExtrasStore.listDocs('site');
    const companyChars = semanticExtrasStore.totalChars('company');
    const siteChars = semanticExtrasStore.totalChars('site');
    const body = `
    ${hubCss()}
    ...
    <div class="hub-grid">
      ...
      ${hubTile({ title: 'Company info', href: '/admin/semantic-store/company', count: companyDocs.length, blurb: 'Talent Transformation identity — mission, methodology, tone of voice, case studies. ~' + companyChars.toLocaleString('en-US') + ' chars indexed.' })}
      ${hubTile({ title: 'Website content', href: '/admin/semantic-store/site', count: siteDocs.length, blurb: 'Blog posts, articles, page copy from talenttransformation.com — pasted or scraped body. ~' + siteChars.toLocaleString('en-US') + ' chars indexed.' })}
    </div>
    <div class="painel" style="margin-top:18px">
      <h3>What belongs here</h3>
      <ul style="margin:6px 0 0 18px;color:var(--ink2);font-size:13px;line-height:1.6">
        <li>Anything <em>long</em> and <em>semantic</em>: framework theory, course lessons, quiz descriptions, company mission, blog / article body.</li>
...
function semanticExtrasIndexPage(kind, savedMessage = null) {
    if (!semanticExtrasStore.isValidKind(kind)) {
        return shellChrome('Semantic Store', `<div class="painel"><p style="color:var(--muted)">Unknown category. <a href="/admin/semantic-store">Back to Semantic Store</a></p></div>`);
    }
    const label = semanticExtrasStore.kindLabel(kind);
    const blurb = semanticExtrasStore.kindBlurb(kind);
    const docs = semanticExtrasStore.listDocs(kind);
    const banner = savedMessage
        ...
        : '';
    const rowsHtml = docs.length ? docs.map((d) => {
        const c = semanticExtrasStore.readContent(kind, d.doc_id);
        const chars = c ? c.chars : 0;
        const m = d.meta || {};
        return `<tr>
          <td style="padding:10px 12px">
            <a href="/admin/semantic-store/${encodeURIComponent(kind)}/${encodeURIComponent(d.doc_id)}" style="color:var(--ink);text-decoration:none;font-size:13.5px;font-weight:600">${escapeHtml(m.title || d.doc_id)}</a>
            <div style="font-size:11px;color:var(--muted);margin-top:2px">${escapeHtml(m.description || '').slice(0, 140)}</div>
          </td>
          <td style="text-align:right;font-size:12px;color:var(--muted);padding:10px 12px">${chars.toLocaleString('en-US')} chars</td>
          <td style="padding:10px 12px">
            <form method="POST" action="/admin/semantic-store/${encodeURIComponent(kind)}/${encodeURIComponent(d.doc_id)}/delete" onsubmit="return confirm('Delete document ${escapeHtml((m.title || d.doc_id).replace(/'/g, ''))}?')" style="display:inline">
              <button type="submit" style="background:transparent;border:0;color:var(--muted);font-size:16px;cursor:pointer" title="Delete">✕</button>
            </form>
          </td>
        </tr>`;
    }).join('') : `<tr><td colspan="3" style="padding:22px;text-align:center;color:var(--muted);font-size:13px">No ${escapeHtml(kind)} documents yet — add the first one below.</td></tr>`;

    const body = `
    <div style="display:flex;gap:14px;align-items:baseline;margin-bottom:12px;flex-wrap:wrap">
      <a href="/admin/semantic-store" class="botao" style="padding:5px 12px;font-size:12px">← Semantic Store</a>
      <h2 style="margin:0;font-size:19px">${escapeHtml(label)}</h2>
      <span style="font-size:12.5px;color:var(--muted)">${docs.length} document${docs.length === 1 ? '' : 's'}</span>
    </div>
    ${banner}
    <div class="painel">
      <p style="font-size:13px;color:var(--ink2);margin:0 0 10px">${escapeHtml(blurb)}</p>
      <table style="width:100%;border-collapse:collapse">
        <thead><tr>
          <th style="text-align:left;padding:6px 12px;font-size:11px;text-transform:uppercase;letter-spacing:.06em;color:var(--muted);border-bottom:1px solid var(--line)">Title · description</th>
          <th style="text-align:right;padding:6px 12px;font-size:11px;text-transform:uppercase;letter-spacing:.06em;color:var(--muted);border-bottom:1px solid var(--line)">Chars</th>
    ...
    <div class="painel">
      <h3>Add a new ${escapeHtml(kind)} document</h3>
      <form method="POST" action="/admin/semantic-store/${encodeURIComponent(kind)}/create" style="display:grid;gap:10px;max-width:520px">
        <label style="font-size:12px;color:var(--muted);display:flex;flex-direction:column;gap:4px">Document id (slug: a-z 0-9 dash / underscore)
          <input type="text" name="doc_id" required pattern="[a-z0-9][a-z0-9_-]*" maxlength="64" placeholder="our-methodology" style="padding:8px 10px;border:1px solid var(--line);border-radius:6px;background:var(--card);color:var(--ink);font-size:13px">
        </label>
        <label style="font-size:12px;color:var(--muted);display:flex;flex-direction:column;gap:4px">Title
          <input type="text" name="title" required maxlength="200" placeholder="Our methodology" style="padding:8px 10px;border:1px solid var(--line);border-radius:6px;background:var(--card);color:var(--ink);font-size:13px">
        </label>
        <label style="font-size:12px;color:var(--muted);display:flex;flex-direction:column;gap:4px">One-line description
          <input type="text" name="description" maxlength="240" placeholder="How TT designs coaching experiences" style="padding:8px 10px;border:1px solid var(--line);border-radius:6px;background:var(--card);color:var(--ink);font-size:13px">
        </label>
        ${kind === 'site' ? `<label style="font-size:12px;color:var(--muted);display:flex;flex-direction:column;gap:4px">Source URL (optional)
          <input type="url" name="source_url" maxlength="500" placeholder="https://talenttransformation.com/…" style="padding:8px 10px;border:1px solid var(--line);border-radius:6px;background:var(--card);color:var(--ink);font-size:13px">
        </label>` : ''}
        <button type="submit" style="padding:8px 16px;background:var(--accent);color:#fff;border:0;border-radius:6px;cursor:pointer;font-weight:600;font-size:13px;justify-self:start">+ Create</button>
      </form>
    </div>`;
    return shellChrome('Semantic Store · ' + label, body);
}

function semanticExtrasDetailPage(kind, docId, savedMessage = null) {
    if (!semanticExtrasStore.isValidKind(kind) || !semanticExtrasStore.isValidDocId(docId)) {
        return shellChrome('Semantic Store', `<div class="painel"><p style="color:var(--muted)">Invalid document. <a href="/admin/semantic-store">Back to Semantic Store</a></p></div>`);
    }
    const meta = semanticExtrasStore.readMeta(kind, docId);
    if (!meta) {
        return shellChrome('Semantic Store', `<div class="painel"><p style="color:var(--muted)">Document not found. <a href="/admin/semantic-store/${encodeURIComponent(kind)}">Back to ${escapeHtml(semanticExtrasStore.kindLabel(kind))}</a></p></div>`);
    }
    const content = semanticExtrasStore.readContent(kind, docId);
    const kindLabel = semanticExtrasStore.kindLabel(kind);
    const banner = savedMessage
        ? `<div style="padding:10px 14px;background:var(--accent-tint);border-left:3px solid var(--accent);border-radius:8px;margin-bottom:12px;font-size:13.5px">${escapeHtml(savedMessage)}</div>`
        : '';
    const body = `
    <div style="display:flex;gap:14px;align-items:baseline;margin-bottom:12px;flex-wrap:wrap">
      <a href="/admin/semantic-store/${encodeURIComponent(kind)}" class="botao" style="padding:5px 12px;font-size:12px">← ${escapeHtml(kindLabel)}</a>
      <h2 style="margin:0;font-size:19px">${escapeHtml(meta.title || docId)}</h2>
      <span style="font-size:11.5px;color:var(--muted);letter-spacing:.06em;text-transform:uppercase">doc_id: ${escapeHtml(docId)}</span>
      <span style="margin-left:auto;font-size:12px;color:var(--muted)">${(content?.chars || 0).toLocaleString('en-US')} chars</span>
    </div>
    ${banner}
    <div class="painel">
      <h3>Metadata</h3>
      <form method="POST" action="/admin/semantic-store/${encodeURIComponent(kind)}/${encodeURIComponent(docId)}/meta" style="display:grid;gap:10px;max-width:640px">
        <label style="font-size:12px;color:var(--muted);display:flex;flex-direction:column;gap:4px">Title
          <input type="text" name="title" value="${escapeHtml(meta.title || '')}" required maxlength="200" style="padding:8px 10px;border:1px solid var(--line);border-radius:6px;background:var(--card);color:var(--ink);font-size:13px">
        </label>
        <label style="font-size:12px;color:var(--muted);display:flex;flex-direction:column;gap:4px">Description
          <input type="text" name="description" value="${escapeHtml(meta.description || '')}" maxlength="240" style="padding:8px 10px;border:1px solid var(--line);border-radius:6px;background:var(--card);color:var(--ink);font-size:13px">
        </label>
        ${kind === 'site' ? `<label style="font-size:12px;color:var(--muted);display:flex;flex-direction:column;gap:4px">Source URL
          <input type="url" name="source_url" value="${escapeHtml(meta.source_url || '')}" maxlength="500" style="padding:8px 10px;border:1px solid var(--line);border-radius:6px;background:var(--card);color:var(--ink);font-size:13px">
        </label>` : ''}
        <button type="submit" style="padding:8px 16px;background:var(--accent);color:#fff;border:0;border-radius:6px;cursor:pointer;font-weight:600;font-size:13px;justify-self:start">Save metadata</button>
      </form>
    </div>
    <div class="painel">
      <h3>Content (markdown)</h3>
      <p style="font-size:12.5px;color:var(--muted);margin:0 0 8px">Erica searches this content via <code>search_knowledge</code>. Paste plain markdown / text; images or attachments aren't indexed.</p>
      <form method="POST" action="/admin/semantic-store/${encodeURIComponent(kind)}/${encodeURIComponent(docId)}/content" style="display:flex;flex-direction:column;gap:10px">
        <textarea name="text" rows="18" style="font:12.5px/1.55 ui-monospace,Consolas,monospace;padding:12px 14px;border:1px solid var(--line);border-radius:8px;background:var(--soft);color:var(--ink);width:100%">${escapeHtml(content?.text || '')}</textarea>
        <div style="display:flex;gap:10px;align-items:center">
          <button type="submit" style="padding:8px 16px;background:var(--accent);color:#fff;border:0;border-radius:6px;cursor:pointer;font-weight:600;font-size:13px">Save content</button>
          <span style="font-size:11px;color:var(--muted)">Saves persist to the volume immediately. Vector-store re-index is a next step.</span>
        </div>
      </form>
    </div>
    <div class="painel" style="border-color:rgba(208,59,59,.3)">
      <h3 style="color:var(--crit)">Danger zone</h3>
      <form method="POST" action="/admin/semantic-store/${encodeURIComponent(kind)}/${encodeURIComponent(docId)}/delete" onsubmit="return confirm('Delete this ${escapeHtml(kind)} document?')">
        <button type="submit" class="botao" style="color:var(--crit);border-color:var(--crit)">Delete document</button>
      </form>
    </div>`;
    return shellChrome('Semantic Store · ' + (meta.title || docId), body);
}

function injectedDataPage() {
    const frameworks = contentStore.listFrameworks();
    const kinds = injectedDataStore.kindList();
    const chars = injectedDataStore.computeChars();
    const rowsByKind = {};
    let totalRows = 0;
    for (const k of kinds) {
        rowsByKind[k.id] = injectedDataStore.readAll(k.id);
        totalRows += rowsByKind[k.id].length;
    }
    const injectedTiles = kinds.map((k) => {
        const rows = rowsByKind[k.id];
        return hubTile({
            title: k.id === 'safety-rules' ? 'Safety rules'
                : k.id === 'canonical-courses' ? 'Canonical courses'
                : 'Canonical quizzes',
            href: '/admin/injected-data/' + k.id,
            count: rows.length,
            blurb: k.blurb + ' · adds ~' + chars[k.id] + ' chars to the system prompt.'
        });
    }).join('');
    const body = `
    ${hubCss()}
    ...
    <div class="painel" style="margin-top:18px">
      ...
      <ul style="margin:6px 0 0 18px;color:var(--ink2);font-size:13px;line-height:1.6">
        ...
        <li>Total <strong>${totalRows}</strong> canonical entries; combined they add roughly <strong>${chars['canonical-courses'] + chars['canonical-quizzes'] + chars['safety-rules']}</strong> chars to Erica's prompt.</li>
      </ul>
    </div>`;
    return shellChrome('Injected Data', body);
}

function injectedDataEditorPage(kind, savedMessage = null) {
    if (!injectedDataStore.kindExists(kind)) {
        return shellChrome('Injected Data', `<div class="painel"><p style="color:var(--muted)">Unknown injected data kind: <code>${escapeHtml(kind)}</code>. <a href="/admin/injected-data">Back to Injected Data</a></p></div>`);
    }
    const meta = injectedDataStore.kindMeta(kind);
    const rows = injectedDataStore.readAll(kind);
    const previewBlock = injectedDataStore.computeSystemPromptBlock(kind);
    const previewChars = previewBlock.length;

    const banner = savedMessage
        ? `<div style="padding:10px 14px;background:var(--accent-tint);border-left:3px solid var(--accent);border-radius:8px;margin-bottom:12px;font-size:13.5px">${escapeHtml(savedMessage)}</div>`
        : '';

    // Field labels for the table headers + form.
    const labels = {
        course_id: 'ID', quiz_id: 'ID', rule_id: 'Rule ID',
        name: 'Name', url: 'URL', one_line: 'One-liner',
        trigger_hint: 'Trigger', prescribed_response: 'Prescribed response',
        enabled: 'On'
    };

    const displayName = kind === 'safety-rules' ? 'Safety rules'
        : kind === 'canonical-courses' ? 'Canonical courses'
        : 'Canonical quizzes';

    const rowsHtml = rows.length ? rows.map((r) => `
      <tr>
        <td class="idc-row-cells" style="padding:8px 10px">
          <form method="POST" action="/admin/injected-data/${encodeURIComponent(kind)}/upsert" class="idc-row-form">
            ${meta.fields.map((f) => {
                if (f === 'enabled') {
                    return `<td class="idc-cell idc-enabled"><label class="idc-toggle"><input type="checkbox" name="enabled" value="1" ${r.enabled ? 'checked' : ''}><span></span></label></td>`;
                }
                const val = r[f] == null ? '' : r[f];
                return `<input type="${f === 'url' ? 'url' : 'text'}" name="${escapeHtml(f)}" value="${escapeHtml(val)}" ${f === meta.primaryKey ? 'readonly' : ''} class="idc-input ${f === meta.primaryKey ? 'idc-key' : ''}" placeholder="${escapeHtml(labels[f] || f)}">`;
            }).join('')}
            <button type="submit" class="idc-save">save</button>
          </form>
        </td>
        <td class="idc-actions">
          <form method="POST" action="/admin/injected-data/${encodeURIComponent(kind)}/delete" onsubmit="return confirm('Delete this ${escapeHtml(meta.rowNoun)}?')" style="display:inline">
            <input type="hidden" name="${meta.primaryKey}" value="${escapeHtml(r[meta.primaryKey] || '')}">
            <button type="submit" class="idc-del" title="Delete">✕</button>
          </form>
        </td>
      </tr>
    `).join('') : `<tr><td colspan="2" style="padding:22px;text-align:center;color:var(--muted);font-size:13px">No ${escapeHtml(meta.rowNoun)}s yet — add the first one below.</td></tr>`;

    const newFormFields = meta.fields.map((f) => {
        if (f === 'enabled') {
            return `<label class="idc-inline-toggle"><input type="checkbox" name="enabled" value="1" checked><span>On</span></label>`;
        }
        return `<input type="${f === 'url' ? 'url' : 'text'}" name="${escapeHtml(f)}" placeholder="${escapeHtml(labels[f] || f)}" class="idc-input" ${f === meta.primaryKey ? 'required' : ''}>`;
    }).join('');

    const body = `
    <style>
      .idc-hero { display:flex; gap:14px; align-items:baseline; margin-bottom:12px; flex-wrap:wrap; }
      .idc-hero h2 { margin:0; font-size:19px; }
      .idc-hero .meta { font-size:12.5px; color:var(--muted); }
      .idc-hero .chars-chip { font:600 11px/1 inherit; padding:4px 10px; border-radius:999px;
        background:var(--accent-tint); color:var(--accent); }
      .idc-table { width:100%; border-collapse:collapse; }
      .idc-table th { text-align:left; padding:6px 10px; font-size:11px; text-transform:uppercase;
        letter-spacing:.06em; color:var(--muted); border-bottom:1px solid var(--line); }
      .idc-table tr { border-bottom:1px solid var(--line); }
      .idc-row-form { display:grid; grid-template-columns: repeat(${meta.fields.length}, 1fr) auto; gap:6px; align-items:center; margin:0; }
      .idc-input { padding:6px 10px; font-size:13px; border:1px solid var(--line); border-radius:6px;
        background:var(--card); color:var(--ink); width:100%; }
      .idc-input:focus { outline:none; border-color:var(--accent); }
      .idc-input.idc-key { background:var(--soft); color:var(--muted); font-family:ui-monospace,Consolas,monospace; font-size:12px; }
      .idc-save { font-size:11.5px; padding:5px 12px; border:1px solid var(--line); border-radius:6px;
        background:var(--card); color:var(--ink2); cursor:pointer; }
      .idc-save:hover { color:var(--accent); border-color:var(--accent); }
      .idc-del { background:transparent; border:0; color:var(--muted); font-size:16px; cursor:pointer; padding:2px 8px; }
      .idc-del:hover { color:var(--crit); }
      .idc-actions { text-align:right; padding:8px 6px; }
      .idc-toggle { position:relative; display:inline-block; width:34px; height:20px; }
      .idc-toggle input { opacity:0; width:0; height:0; }
      .idc-toggle span { position:absolute; top:0; left:0; right:0; bottom:0; background:var(--line);
        border-radius:20px; transition: background .15s; cursor:pointer; }
      .idc-toggle span::before { content:""; position:absolute; top:2px; left:2px; width:16px; height:16px;
        background:#fff; border-radius:50%; transition: transform .15s; }
      .idc-toggle input:checked + span { background:var(--accent); }
      .idc-toggle input:checked + span::before { transform: translateX(14px); }
      .idc-inline-toggle { display:inline-flex; align-items:center; gap:6px; font-size:12px; color:var(--muted); }
      .idc-new-form { display:grid; grid-template-columns: repeat(${meta.fields.length}, 1fr) auto; gap:6px;
        align-items:center; padding:10px 6px; }
      .idc-new-form button { padding:6px 14px; font-size:12.5px; background:var(--accent); color:#fff;
        border:0; border-radius:6px; cursor:pointer; font-weight:600; }
      .idc-preview { margin-top:20px; }
      .idc-preview pre { background:var(--soft); border:1px solid var(--line); border-radius:8px;
        padding:12px 14px; font:12px/1.55 ui-monospace,Consolas,monospace; white-space:pre-wrap;
        color:var(--ink2); overflow-x:auto; max-height:280px; }
    </style>
    <div class="idc-hero">
      <a href="/admin/injected-data" class="botao" style="padding:5px 12px;font-size:12px">← Injected Data</a>
      <h2>${escapeHtml(displayName)}</h2>
      <span class="meta">${rows.length} row${rows.length === 1 ? '' : 's'}</span>
      <span class="chars-chip">~${previewChars} chars in the prompt</span>
    </div>
    ${banner}
    <div class="painel">
      <h3>Entries</h3>
      <p style="font-size:13px;color:var(--ink2);margin:0 0 8px">${escapeHtml(meta.blurb)}</p>
      <table class="idc-table">
        <thead><tr><th>${meta.fields.map((f) => escapeHtml(labels[f] || f)).join('</th><th>')}</th><th style="text-align:right">Actions</th></tr></thead>
        <tbody>${rowsHtml}</tbody>
      </table>
      <form method="POST" action="/admin/injected-data/${encodeURIComponent(kind)}/upsert" class="idc-new-form">
        ${newFormFields}
        <button type="submit">+ Add</button>
      </form>
    </div>
    <div class="painel idc-preview">
      <h3>System prompt preview</h3>
      <p style="font-size:12.5px;color:var(--muted);margin:0 0 8px">This is the exact block Erica will receive on her next session boot. Empty = nothing injected.</p>
      <pre>${escapeHtml(previewBlock || '(empty — nothing to inject yet)')}</pre>
    </div>`;
    return shellChrome('Injected Data · ' + displayName, body);
}
...
function renderCoachIdentityCard(budget, { coursesCount, quizzesCount, frameworksCount }) {
    ...
    const legend = blocks.map((b, i) => `<div class="ci-legend-row" title="${escapeHtml(b.note || '')}">
      ...
      <span class="ci-legend-info" title="${escapeHtml(b.note || '')}">?</span>
    </div>`).join('');

    // Connected capabilities — was flat chips, now a formal list with icon,
    // label + short blurb, and a chevron to the settings surface.
    const capIcons = {
        preparation: '⚡',
        activity_timeline: '📈',
        user_report: '👤',
    ...
    const caps = (budget.channelsList || []).map((ch) => {
        const on = !!ch.enabled;
        const icon = capIcons[ch.id] || '•';
        const shortName = ch.name.replace(/\s*\([^)]*\)/, '');
        const parens = (ch.name.match(/\(([^)]+)\)/) || [])[1] || '';
        return `<a class="ci-cap ${on ? 'on' : 'off'}" href="/admin/realtime" title="${escapeHtml(ch.purpose || '')}">
          <span class="ci-cap-ico" aria-hidden="true">${icon}</span>
          <span class="ci-cap-body">
            <span class="ci-cap-name">${escapeHtml(shortName)}</span>
            ${parens ? `<span class="ci-cap-sub">${escapeHtml(parens)}</span>` : ''}
          </span>
          <span class="ci-cap-chev">›</span>
        </a>`;
    }).join('');
    ...
    const lastSess = budget.lastRealSession
        ? `<a href="/admin/sessions/${encodeURIComponent(budget.lastRealSession.sessionId)}" class="ci-last-link">Last active ${escapeHtml((budget.lastRealSession.at || '').slice(11, 16))} UTC</a>`
        : `<span class="ci-last-empty">Waiting for first live session</span>`;

    return `<style>
      /* Slim identity strip: ● Erica in production · [persona chip] · Healthy · Last active */
      .ci-strip { display:flex; align-items:center; gap:14px; padding:14px 18px;
        background:var(--card); border:1px solid var(--line); border-radius:12px;
        margin: 4px 0 18px; box-shadow: 0 1px 3px var(--ring); flex-wrap:wrap; }
      .ci-strip .live-dot { width:9px; height:9px; border-radius:50%; background:#22c55e;
        box-shadow: 0 0 0 4px rgba(34,197,94,.18); flex:none; }
      .ci-strip .strip-name { font: 600 15.5px/1 "Inter",sans-serif; color:var(--ink); }
      .ci-strip .strip-persona { font:600 11px/1 inherit; padding:5px 10px; border-radius:999px;
        background:var(--accent-tint); color:var(--accent); text-decoration:none;
        border:1px solid rgba(20,184,166,.28); }
      .ci-strip .strip-persona:hover { background:rgba(20,184,166,.14); text-decoration:none; }
      .ci-strip .strip-health { display:inline-flex; align-items:center; gap:6px; font-size:12.5px;
        color:#0ca30c; font-weight:500; }
      .ci-strip .strip-health::before { content:""; width:6px; height:6px; border-radius:50%;
        background:#0ca30c; }
      .ci-strip .strip-last { margin-left:auto; font-size:12.5px; color:var(--muted); }
      .ci-strip .strip-last a { color:var(--muted); text-decoration:none; border-bottom:1px dashed var(--line); }
      .ci-strip .strip-last a:hover { color:var(--accent); border-color:var(--accent); }

      /* Two-column card row: Prompt composition (wide) | Connected capabilities */
      .ci-row { display:grid; grid-template-columns: minmax(360px, 1.5fr) minmax(280px, 1fr); gap:16px; margin-bottom:16px; }
      @media (max-width:860px) { .ci-row { grid-template-columns: 1fr; } }
      .ci-card { background:var(--card); border:1px solid var(--line); border-radius:12px;
        padding:18px 20px; box-shadow: 0 1px 3px var(--ring); display:flex; flex-direction:column; }
      .ci-card-head { display:flex; align-items:baseline; justify-content:space-between; gap:10px; margin-bottom:14px; }
      .ci-card-head h3 { margin:0; font: 600 14.5px/1.2 "Inter",sans-serif; color:var(--ink); }
      .ci-card-head .ci-card-hint { font-size:11.5px; color:var(--muted); }
      .ci-card-head .ci-card-cap { font-size:12.5px; color:var(--muted); }
      .ci-card-head .ci-card-cap b { color:var(--ink2); font-weight:600; }
      .ci-bar-track { height:14px; border-radius:7px; background:var(--soft); overflow:hidden; display:flex;
        border:1px solid var(--line); }
      .ci-seg { height:100%; }
      .ci-bar-meta { display:flex; justify-content:space-between; margin-top:8px; font-size:12px; color:var(--muted);
        font-variant-numeric: tabular-nums; }
      .ci-bar-meta .ci-total { color:var(--ink2); font-weight:600; }
      .ci-legend { margin-top:12px; display:flex; flex-direction:column; gap:2px; }
      .ci-legend-row { display:grid; grid-template-columns: 12px 1fr auto auto 18px; gap:8px; align-items:center;
        font-size:12px; padding:4px 2px; cursor:help; border-radius:5px; }
      .ci-legend-row:hover { background:rgba(20, 184, 166, 0.05); }
      .ci-legend-info { display:inline-flex; width:16px; height:16px; align-items:center; justify-content:center;
        border-radius:50%; background:var(--soft); color:var(--muted); font:600 10px/1 inherit; cursor:help; }
      .ci-legend-info:hover { color:var(--accent); background:var(--accent-tint); }
      ...
      .ci-src-measured { color:var(--accent); border-color:rgba(20,184,166,.35); background:rgba(20,184,166,.06); }
      .ci-src-reported { color:var(--accent); border-color:rgba(20,184,166,.28); background:rgba(20,184,166,.04); }
      .ci-src-estimated { color:#b45309; border-color:rgba(180,83,9,.28); background:rgba(180,83,9,.05); }
      .ci-src-off { background:var(--soft); color:var(--muted); }
      .ci-src-missing { color:var(--crit); border-color:rgba(208,59,59,.28); background:rgba(208,59,59,.05); }

      /* Connected capabilities list */
      .ci-caps { display:flex; flex-direction:column; }
      .ci-cap { display:grid; grid-template-columns:32px 1fr auto; gap:10px; align-items:center;
        padding:10px 6px; border-bottom:1px solid var(--line); text-decoration:none; color:inherit;
        transition: background .12s ease; }
      .ci-cap:last-child { border-bottom:0; }
      .ci-cap:hover { background:rgba(13,148,136,.04); text-decoration:none; }
      .ci-cap.off { opacity:.55; }
      .ci-cap-ico { width:28px; height:28px; border-radius:8px; background:var(--soft); display:flex;
        align-items:center; justify-content:center; font-size:14px; color:var(--accent); }
      .ci-cap.off .ci-cap-ico { color:var(--muted); }
      .ci-cap-body { display:flex; flex-direction:column; line-height:1.3; min-width:0; }
      .ci-cap-name { font-size:13.5px; color:var(--ink); font-weight:500; overflow:hidden;
        text-overflow:ellipsis; white-space:nowrap; }
      .ci-cap-sub { font-size:11.5px; color:var(--muted); overflow:hidden; text-overflow:ellipsis;
        white-space:nowrap; }
      .ci-cap-chev { color:var(--muted); font-size:18px; line-height:1; }

      /* Corpus + actions footer */
      .ci-footer { display:flex; align-items:center; gap:16px; padding:14px 20px; border:1px solid var(--line);
        border-radius:12px; background:var(--card); margin-bottom:22px; box-shadow: 0 1px 3px var(--ring);
        flex-wrap:wrap; }
      .ci-corpus { display:flex; gap:16px; }
      ...
      .ci-corpus .n { font-weight:700; color:var(--ink); font-variant-numeric: tabular-nums; margin-right:4px; }
      .ci-manage { font-size:12.5px; color:var(--muted); text-decoration:none; border-bottom:1px dashed var(--line); }
      .ci-manage:hover { color:var(--accent); border-color:var(--accent); }
      .ci-actions { display:flex; gap:8px; margin-left:auto; flex-wrap:wrap; }
      .ci-actions a { font-size:12.5px; padding:7px 14px; border-radius:8px; border:1px solid var(--line);
        color:var(--ink2); background:var(--card); text-decoration:none; font-weight:500;
        transition: all .12s ease; }
      .ci-actions a.primary { background:var(--accent); color:#fff; border-color:transparent; }
      .ci-actions a:hover { border-color:var(--accent); color:var(--accent); }
      .ci-actions a.primary:hover { color:#fff; filter:brightness(1.05); }
    </style>
    <div class="ci-strip">
      <span class="live-dot" aria-hidden="true"></span>
      <span class="strip-name">Erica in production</span>
      <a class="strip-persona" href="/admin/frameworks/${encodeURIComponent(personaId)}">${escapeHtml(persona)}</a>
      <span class="strip-health">Healthy</span>
      <span class="strip-last">${lastSess}</span>
    </div>
    <div class="ci-row">
      <div class="ci-card">
        <div class="ci-card-head">
          <h3>Prompt composition</h3>
          <span class="ci-card-cap">${pctBar}% of <b>${budget.budget.toLocaleString('en-US')}</b> cap</span>
        </div>
        <div class="ci-card-head" style="margin:-8px 0 10px; font-size:12px; color:var(--muted); justify-content:flex-start; gap:4px">
        ...
        <div class="ci-bar-meta">
          ...
          <span class="ci-total ${pctTone === 'ok' ? '' : pctTone}">${pctBar}% used</span>
        </div>
        <div class="ci-legend">${legend}</div>
      </div>
      <div class="ci-card" style="padding:16px 4px 8px">
        <div class="ci-card-head" style="padding:0 16px">
          <h3>Connected capabilities</h3>
          <span class="ci-card-hint">${(budget.channelsList || []).filter(ch => ch.enabled).length}/${(budget.channelsList || []).length} active</span>
        </div>
        <div class="ci-caps" style="padding:0 12px">${caps}</div>
      </div>
    </div>
    <div class="ci-footer">
      <div class="ci-corpus">
      ...
      <a href="/admin/realtime" class="ci-manage">Manage connections →</a>
      <div class="ci-actions">
        <a href="/admin/simulator" class="primary">Open simulator ↗</a>
...
function indexPage(session) {
    ...
    // Sparkline SVG — 14-day daily series. Tint controls the colour tone.
    // series: array of numbers (any length; we sample the last 14).
    function sparkline(series, tint = 'ok') {
        const data = (series || []).slice(-14);
        if (!data.length) return '';
        const W = 120, H = 26, padY = 3;
        const max = Math.max(1, ...data);
        const step = data.length > 1 ? (W / (data.length - 1)) : 0;
        const y = (v) => (H - padY) - ((v / max) * (H - 2 * padY));
        const pts = data.map((v, i) => (i * step) + ',' + y(v).toFixed(1)).join(' ');
        const areaPts = pts + ' ' + W + ',' + H + ' 0,' + H;
        const colour = tint === 'warn' ? '#f59e0b' : tint === 'crit' ? '#d03b3b' : '#0d9488';
        const gradId = 'sg-' + tint;
        return `<svg viewBox="0 0 ${W} ${H}" width="100%" height="${H}" preserveAspectRatio="none" style="display:block;margin-top:6px" role="img">
          <defs><linearGradient id="${gradId}" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0" stop-color="${colour}" stop-opacity=".35"/>
    ...
    const dailySessions = (m.daily || []).map(d => d.sessions || 0);
    const dailyToolErrors = (m.daily || []).map(d => d.toolErrors || 0);

    // Usage / Issues sections built as clean two-row card grids with sparklines.
    const usageCard = (rot, val, sub, series, tint) => `<div class="card sect-card">
      <div class="rot">${escapeHtml(rot)}</div>
      <div class="val">${escapeHtml(String(val))}</div>
      <div class="var">${escapeHtml(sub || '')}</div>
      ${sparkline(series, tint)}
    </div>`;
    const issueCard = (rot, val, hint, tone, series) => `<div class="card sect-card${tone ? ' sect-' + tone : ''}">
      <div class="rot">${escapeHtml(rot)}</div>
      <div class="val"${tone ? ` style="color:var(--${tone})"` : ''}>${escapeHtml(String(val))}</div>
      <div class="var">${escapeHtml(hint || '')}</div>
      ${sparkline(series, tone === 'crit' ? 'crit' : tone === 'warn' ? 'warn' : 'ok')}
    </div>`;
        ...
        ? recent.map((s) => {
            ...
            return `<tr>
              <td style="max-width:340px">
                <a href="/admin/sessions/${encodeURIComponent(s.sessionId)}" style="color:var(--ink);text-decoration:none;font-size:13.5px;line-height:1.35;display:block">${escapeHtml(label)}</a>
                <div style="font-size:10.5px;color:var(--muted);margin-top:2px">${escapeHtml(actor)} · ${s.turns} turn${s.turns === 1 ? '' : 's'}</div>
              </td>
              <td style="white-space:nowrap;font-size:12px;color:var(--ink2)">${escapeHtml(_formatSessionTime(s.lastAt))}</td>
            </tr>`;
        }).join('')
        : '<tr><td colspan="2" style="text-align:center;color:var(--muted);padding:18px 0">no sessions yet — trigger one from the coach to populate.</td></tr>';
    ...
    const body = `
    <div class="titulo">
      <div><h2>${greet}, ${escapeHtml(session.e || session.u || session.sub)}</h2>
    ...
    <style>
      .sect-title { display:flex; align-items:baseline; justify-content:space-between;
        margin: 20px 0 10px; }
      .sect-title h3 { margin:0; font: 600 14.5px/1.2 "Inter",sans-serif; color:var(--ink); letter-spacing:-.005em; }
      .sect-title .hint { font-size:12px; color:var(--muted); }
      .sect-grid { display:grid; grid-template-columns:repeat(auto-fit, minmax(200px, 1fr)); gap:12px; margin-bottom:22px; }
      .sect-card { background:var(--card); border:1px solid var(--line); border-radius:12px;
        padding:14px 16px; display:flex; flex-direction:column; gap:2px; box-shadow: 0 1px 3px var(--ring);
        transition: border-color .15s, box-shadow .15s; }
      .sect-card:hover { border-color:var(--line-hi, var(--ring-hi)); box-shadow: 0 4px 12px -4px var(--ring); }
      .sect-card .rot { font: 600 10.5px/1 "Inter",sans-serif; text-transform:uppercase; letter-spacing:.08em;
        color:var(--muted); }
      .sect-card .val { font: 600 26px/1.1 "Inter",sans-serif; color:var(--ink); font-variant-numeric: tabular-nums;
        margin-top:4px; letter-spacing:-.02em; }
      .sect-card .var { font-size:12px; color:var(--muted); }
      .sect-card.sect-warn { border-color:rgba(245,158,11,.35); }
      .sect-card.sect-crit { border-color:rgba(208,59,59,.4); }
    </style>
    <div class="sect-title"><h3>Usage</h3><span class="hint">last 14 days</span></div>
    <div class="sect-grid">
      ${usageCard('Last 24h', m.volume.last24h.sessions, m.volume.last24h.turns + ' turns · ' + m.volume.last24h.tools + ' tools', dailySessions.slice(-1), 'ok')}
      ${usageCard('Last 7 days', m.volume.last7d.sessions, m.volume.last7d.turns + ' turns · ' + m.volume.last7d.tools + ' tools', dailySessions.slice(-7), 'ok')}
      ${usageCard('Last 30 days', m.volume.last30d.sessions, m.volume.last30d.turns + ' turns · ' + m.volume.last30d.tools + ' tools', dailySessions, 'ok')}
    </div>
    <div class="sect-title"><h3>Issues</h3><span class="hint">quality signals to watch</span></div>
    <div class="sect-grid">
      ${issueCard('Rage close', m.qualitySignals.rageClose, 'short sessions ending on user', m.qualitySignals.rageClose > 0 ? 'warn' : null, dailySessions)}
      ${issueCard('Long silence', m.qualitySignals.longSilenceCount, 'gaps > 90s', m.qualitySignals.longSilenceCount > 5 ? 'warn' : null, dailySessions)}
      ${issueCard('Tool failures', m.qualitySignals.toolFailureCount, 'sessions with ≥ 1 error', m.qualitySignals.toolFailureCount > 0 ? 'crit' : null, dailyToolErrors)}
    </div>

    <div class="painel">
      <h3>Recent sessions</h3>
      <table><thead><tr><th>Session · first message</th><th>Last</th></tr></thead><tbody>${recentRows}</tbody></table>
...
async function handle(req, res) {
    const url = req.url || '';
    if (!url.startsWith('/admin') && !url.startsWith('/auth')) return false;

    // Parse cookies once.
    const cookies = parseCookie(req.headers.cookie);
    const session = verifySession(cookies[COOKIE_NAME]);

    // ---- Public: sign-in landing (Wix button) -----------------------------
    if (req.method === 'GET' && (url === '/admin/login' || url.startsWith('/admin/login?'))) {
        const params = new URL(url, 'http://x').searchParams;
        const errorCode = params.get('e');
        const messages = { session_expired: 'Your session expired. Sign in again.' };
        const err = messages[errorCode] || (errorCode ? 'Unable to sign in. Try again.' : null);
        res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
        res.end(loginPage({ error: err }));
        return true;
    }

    // ---- Public: begin Wix OAuth (no-JS fallback) --------------------------
    // GET /auth/login → mint anonymous visitor token, ask Wix for a redirect
    // session, hand the person off to Wix's own login page. The PKCE
    // verifier + state stay in an httpOnly cookie so the callback can
    // verify who came back. Kept as a plain, bookmarkable/no-JS route
    // alongside the JSON /auth/wix/begin the login page's buttons actually
    // use — this one always uses the ordinary Wix dialog (no idp).
    if (req.method === 'GET' && (url === '/auth/login' || url.startsWith('/auth/login?'))) {
        if (!wixOauth.loginConfigured()) {
            res.writeHead(500, { 'Content-Type': 'text/html; charset=utf-8' });
            res.end(loginPage({ error: 'Sign-in is not configured on this deployment (WIX_CLIENT_ID missing).' }));
            return true;
        }
        try {
            const { url: wixUrl, pkce } = await wixOauth.beginLogin();
            res.writeHead(302, {
                Location: wixUrl,
                'Set-Cookie': wixOauth.pkceCookie(pkce),
            });
            res.end();
        } catch (e) {
            res.writeHead(500, { 'Content-Type': 'text/html; charset=utf-8' });
            res.end(loginPage({ error: e.message || 'Could not start sign-in.' }));
        }
        return true;
    }

    // ---- Public: begin Wix OAuth (JSON, used by the login page's buttons) --
    // POST { provider?: 'google' } -> { ok:true, url } | { ok:false, message }.
    // The login page's JS fetches this, sets a per-button "Taking you
    // there…" state, then does a full navigation to `url` itself — a plain
    // <a href> can't show that state before the cross-site jump. Ported
    // shape from PIMS's beginWixSignIn server function (src/lib/pims/repo/
    // wix-auth.ts): a provider name is all the browser sends, resolved to a
    // Wix provider id server-side, never accepted from the client directly.
    if (req.method === 'POST' && url === '/auth/wix/begin') {
        if (!wixOauth.loginConfigured()) {
            res.writeHead(200, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ ok: false, message: 'Sign-in is not configured on this deployment.' }));
            return true;
        }
        let provider;
        try {
            ...
            if (parsed && parsed.provider === 'google') provider = 'google';
        } catch (_) { /* no body / bad JSON -> ordinary dialog */ }
        try {
            const { url: wixUrl, pkce } = await wixOauth.beginLogin(provider);
            res.writeHead(200, {
                'Content-Type': 'application/json',
                'Set-Cookie': wixOauth.pkceCookie(pkce),
            });
            res.end(JSON.stringify({ ok: true, url: wixUrl }));
        } catch (e) {
            res.writeHead(200, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ ok: false, message: e.message || 'Could not start signing in through Talent Transformation.' }));
        }
        return true;
    }

    // ---- Public: OAuth callback (client-side fragment reader) -------------
    // Wix returns `code` and `state` after the '#', so the server never sees
    // them. This HTML runs a tiny script that reads the fragment and POSTs
    // it to /auth/wix/complete on the same origin.
    if (req.method === 'GET' && (url === '/auth/callback' || url.startsWith('/auth/callback?'))) {
...
<body class="min-h-screen bg-gray-50 flex items-center justify-center p-4">
  <div class="w-full max-w-sm bg-white rounded-2xl shadow-sm p-6 space-y-4 border border-gray-200">
    <div class="text-lg font-semibold text-gray-900">Signing you in…</div>
    <p class="text-xs text-gray-500">One moment.</p>
    <p id="err" hidden class="text-sm text-red-600"></p>
    <form id="f" method="POST" action="/auth/wix/complete">
      <input type="hidden" name="code">
      <input type="hidden" name="state">
      <noscript><p class="text-sm text-red-600">This step needs JavaScript, because Wix returns the sign-in in the part of the URL that browsers keep to themselves.</p></noscript>
    </form>
  </div>
  <script>
    (function () {
      ...
      var err = document.getElementById('err');
      if (!code || !state) {
        err.textContent = f.get('errorDescription') || f.get('error') ||
          'Wix did not send back a sign-in to complete.';
        err.hidden = false; return;
      }
      var form = document.getElementById('f');
      form.code.value = code; form.state.value = state;
      form.submit();
    })();
  </script>
</body></html>`;
    ...
    // ---- Public: complete OAuth (exchange code for identity) --------------
    if (req.method === 'POST' && url === '/auth/wix/complete') {
        const body = await readBody(req);
        const code = extractField(body, 'code');
        const state = extractField(body, 'state');
        const stashed = wixOauth.readPkce(cookies[wixOauth.PKCE_COOKIE]);
        try {
            const who = await wixOauth.completeLogin({ code, state, stashed });
            res.writeHead(302, {
                Location: '/admin/',
                'Set-Cookie': [
                    wixOauth.clearPkceCookie(),
                    makeSessionCookie({ memberId: who.memberId, email: who.email, name: who.name }),
                ],
            });
            res.end();
        } catch (e) {
            res.writeHead(401, {
                'Content-Type': 'text/html; charset=utf-8',
                'Set-Cookie': wixOauth.clearPkceCookie(),
            });
            res.end(loginPage({ error: e.message || 'Sign-in could not be completed.' }));
        }
        return true;
    }

    // ---- Public: logout -----------------------------------------------------
    // POST-only so a stray <img src="/auth/logout"> can't sign someone out.
    if (req.method === 'POST' && (url === '/auth/logout' || url === '/admin/logout')) {
        res.writeHead(302, { 'Set-Cookie': clearSessionCookie(), Location: '/admin/login' });
        res.end();
        return true;
    }

    // /auth/* has no other public routes.
    ...
    // Two failure modes: no cookie (bounce to sign-in), or cookie ok but the
    // Wix badge is missing/unreadable (denial page). The badge is re-read
    // per request (60s cache), so revoking access in Wix propagates within
    // a minute.
    if (!session) {
        if (url.startsWith('/admin/ping') || req.headers.accept?.includes('application/json')) {
            ...
            const reason = cookies[COOKIE_NAME] ? '?e=session_expired' : '';
            res.writeHead(302, { Location: `/admin/login${reason}` });
            res.end();
        }
        return true;
    }

    const access = await wixBadges.holdsAdminBadge(session.m);
    if (!access.ok) {
        if (url.startsWith('/admin/ping') || req.headers.accept?.includes('application/json')) {
            res.writeHead(403, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ error: 'forbidden', reason: access.reason }));
        } else {
            res.writeHead(403, { 'Content-Type': 'text/html; charset=utf-8' });
            res.end(denialPage({ email: session.e, reason: access.reason, failed: access.failed }));
        }
        return true;
    }
    ...
    // Protected: Semantic Store extras — company + site index pages.
    if (req.method === 'GET' && url.match(/^\/admin\/semantic-store\/(company|site)(?:\?|$)/)) {
        const kind = url.split('?')[0].slice('/admin/semantic-store/'.length);
        const p = new URL(url, 'http://x').searchParams;
        sendPage(req, res, semanticExtrasIndexPage(kind, p.get('msg') || null));
        return true;
    }

    // Protected: Semantic Store extras — detail page.
    if (req.method === 'GET' && url.match(/^\/admin\/semantic-store\/(company|site)\/[a-z0-9][a-z0-9_-]{0,63}(?:\?|$)/)) {
        const rest = url.split('?')[0].slice('/admin/semantic-store/'.length);
        const [kind, docId] = rest.split('/');
        const p = new URL(url, 'http://x').searchParams;
        sendPage(req, res, semanticExtrasDetailPage(kind, docId, p.get('msg') || null));
        return true;
    }

    // Protected: Semantic Store extras — create.
    if (req.method === 'POST' && url.match(/^\/admin\/semantic-store\/(company|site)\/create$/)) {
        const kind = url.split('?')[0].slice('/admin/semantic-store/'.length).replace(/\/create$/, '');
        const body = await readBody(req);
        const docId = (extractField(body, 'doc_id') || '').toLowerCase();
        try {
            semanticExtrasStore.createDoc(kind, docId, {
                title: extractField(body, 'title'),
                description: extractField(body, 'description'),
                source_url: extractField(body, 'source_url')
            }, { actor: session.sub });
            res.writeHead(302, { Location: '/admin/semantic-store/' + encodeURIComponent(kind) + '/' + encodeURIComponent(docId) });
            res.end();
        } catch (e) {
            res.writeHead(302, { Location: '/admin/semantic-store/' + encodeURIComponent(kind) + '?msg=' + encodeURIComponent('Create failed: ' + (e?.message || e)) });
            res.end();
        }
        return true;
    }

    // Protected: Semantic Store extras — write metadata.
    if (req.method === 'POST' && url.match(/^\/admin\/semantic-store\/(company|site)\/[a-z0-9][a-z0-9_-]{0,63}\/meta$/)) {
        const rest = url.split('?')[0].slice('/admin/semantic-store/'.length).replace(/\/meta$/, '');
        const [kind, docId] = rest.split('/');
        const body = await readBody(req);
        try {
            semanticExtrasStore.writeMeta(kind, docId, {
                title: extractField(body, 'title'),
                description: extractField(body, 'description'),
                source_url: extractField(body, 'source_url')
            }, { actor: session.sub });
            res.writeHead(302, { Location: '/admin/semantic-store/' + encodeURIComponent(kind) + '/' + encodeURIComponent(docId) + '?msg=' + encodeURIComponent('Metadata saved.') });
            res.end();
        } catch (e) {
            res.writeHead(302, { Location: '/admin/semantic-store/' + encodeURIComponent(kind) + '/' + encodeURIComponent(docId) + '?msg=' + encodeURIComponent('Save failed: ' + (e?.message || e)) });
            res.end();
        }
        return true;
    }

    // Protected: Semantic Store extras — write content.
    if (req.method === 'POST' && url.match(/^\/admin\/semantic-store\/(company|site)\/[a-z0-9][a-z0-9_-]{0,63}\/content$/)) {
        const rest = url.split('?')[0].slice('/admin/semantic-store/'.length).replace(/\/content$/, '');
        const [kind, docId] = rest.split('/');
        const body = await readBody(req);
        const text = extractField(body, 'text') || '';
        try {
            const r = semanticExtrasStore.writeContent(kind, docId, text, { actor: session.sub });
            const msg = r.changed ? 'Content saved (' + text.length.toLocaleString('en-US') + ' chars).' : 'No change.';
            res.writeHead(302, { Location: '/admin/semantic-store/' + encodeURIComponent(kind) + '/' + encodeURIComponent(docId) + '?msg=' + encodeURIComponent(msg) });
            res.end();
        } catch (e) {
            res.writeHead(302, { Location: '/admin/semantic-store/' + encodeURIComponent(kind) + '/' + encodeURIComponent(docId) + '?msg=' + encodeURIComponent('Save failed: ' + (e?.message || e)) });
            res.end();
    ...
    // Protected: Semantic Store extras — delete.
    if (req.method === 'POST' && url.match(/^\/admin\/semantic-store\/(company|site)\/[a-z0-9][a-z0-9_-]{0,63}\/delete$/)) {
        const rest = url.split('?')[0].slice('/admin/semantic-store/'.length).replace(/\/delete$/, '');
        const [kind, docId] = rest.split('/');
        try {
            semanticExtrasStore.deleteDoc(kind, docId, { actor: session.sub });
            res.writeHead(302, { Location: '/admin/semantic-store/' + encodeURIComponent(kind) + '?msg=' + encodeURIComponent('Deleted ' + docId + '.') });
            res.end();
        } catch (e) {
            res.writeHead(302, { Location: '/admin/semantic-store/' + encodeURIComponent(kind) + '?msg=' + encodeURIComponent('Delete failed: ' + (e?.message || e)) });
            res.end();
        }
        return true;
    }
    ...
    // Protected: Injected Data per-kind editor.
    if (req.method === 'GET' && url.match(/^\/admin\/injected-data\/[a-z0-9-]+(?:\?|$)/)) {
        const kind = url.split('?')[0].slice('/admin/injected-data/'.length);
        const params = new URL(url, 'http://x').searchParams;
        const msg = params.get('msg') || null;
        sendPage(req, res, injectedDataEditorPage(kind, msg));
        return true;
    }

    // Protected: Injected Data upsert row.
    if (req.method === 'POST' && url.match(/^\/admin\/injected-data\/[a-z0-9-]+\/upsert$/)) {
        const kind = url.split('?')[0].slice('/admin/injected-data/'.length).replace(/\/upsert$/, '');
        const body = await readBody(req);
        try {
            const meta = injectedDataStore.kindMeta(kind);
            const incoming = {};
            for (const f of meta.fields) incoming[f] = extractField(body, f);
            const row = injectedDataStore.upsertRow(kind, incoming, { actor: session.sub });
            const msg = 'Saved ' + row[meta.primaryKey] + '.';
            res.writeHead(302, { Location: '/admin/injected-data/' + encodeURIComponent(kind) + '?msg=' + encodeURIComponent(msg) });
            res.end();
        } catch (e) {
    ...
    // Protected: Injected Data delete row.
    if (req.method === 'POST' && url.match(/^\/admin\/injected-data\/[a-z0-9-]+\/delete$/)) {
        const kind = url.split('?')[0].slice('/admin/injected-data/'.length).replace(/\/delete$/, '');
        const body = await readBody(req);
        try {
            const meta = injectedDataStore.kindMeta(kind);
            const id = extractField(body, meta.primaryKey);
            injectedDataStore.deleteRow(kind, id, { actor: session.sub });
            res.writeHead(302, { Location: '/admin/injected-data/' + encodeURIComponent(kind) + '?msg=' + encodeURIComponent('Deleted ' + id + '.') });
            res.end();
        } catch (e) {
            res.writeHead(302, { Location: '/admin/injected-data/' + encodeURIComponent(kind) + '?msg=' + encodeURIComponent('Delete failed: ' + (e?.message || e)) });
            res.end();
        }
        return true;
    }
...
// Used by /api/admin/* endpoints in server.js. Cookie verification is sync
// but the Wix badge re-read is async (60s cache per member), so this is
// async too. Returns the session payload on success, null on any failure —
// callers just do `if (!sess) return 401`. The /admin/* HTML gate uses the
// richer holdsAdminBadge() directly so it can render a denial page with the
// reason instead of a bare 401.
