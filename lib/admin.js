...
<style>
  * { box-sizing: border-box; }
  ...
  /* Background is Willian's own generated asset (studio-assets/login-hero-bg.webp
     — named to NOT start with "/admin": that prefix is routed to admin.js's
     session-gated handler, which would 302 an unauthenticated request for
     this image straight to /admin/login, i.e. the background would never
     load for the one page that needs it) — real depth/shadow on the three
     dashboard-preview cards that a hand-rolled CSS approximation couldn't
     match, plus the large glow circles at proper intensity. A dark overlay
     (::before) sits between the image and the text so the headline stays
     readable over the image's brighter glow areas without needing to fight
     the image itself for contrast. */
  .hero { position:relative; overflow:hidden; flex:none;
    background: #071D26 url('/studio-assets/login-hero-bg.webp') no-repeat center right / cover;
  ...
  @media (max-width: 899px) { .hero { padding:22px 24px 20px; } }
  .hero::before { content:""; position:absolute; inset:0; pointer-events:none;
    background: linear-gradient(100deg, rgba(4,14,20,.78) 0%, rgba(4,14,20,.55) 38%, rgba(4,14,20,.22) 62%, rgba(4,14,20,.35) 100%); }
  ...
  /* Mobile: the hero used to stack ABOVE the sign-in card at full height —
     kicker + two-line headline + subtitle + wrapped chips + footer kicker —
     pushing the actual form below the fold on a phone (reported: ~890px of
     hero before "Sign in" appeared, on an 812px-tall viewport). The topbar
     already carries the brand mark on every screen size, so on narrow
     viewports the hero's only job is a one-line orientation, not a repeat
     of the full pitch: kicker, subtitle, chips and the footer kicker drop,
     and the headline shrinks to one line. */
  @media (max-width: 899px) {
    .kicker, .hero-copy p, .chiprow, .footkicker { display:none; }
    .hero-copy { margin-top:0; }
    .hero-copy h1 { font-size:19px; line-height:1.35; margin:0; }
...
<body>
  ...
  <div class="hero">
    <div>
      ...
      <!-- The three dashboard-preview cards are baked into
           admin-assets/login-hero-bg.webp itself (real depth/shadow, more
           convincing than the hand-rolled CSS cards this replaced) — no
           DOM needed for them. -->
    </div>
