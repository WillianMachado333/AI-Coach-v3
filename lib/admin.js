/*
 * Coach Studio admin — Wix badge auth (12-Aug-2026).
 *
 ...
 *
 * The previous ADMIN_PASSWORD path is gone: cookies minted without a member
 * id (i.e. old sessions from before this change) fail verifySession's shape
 * check and force a fresh Wix sign-in.
 */
...
function adminEnvironmentLabel() {
    const configured = String(process.env.AI_COACH_ENVIRONMENT || '').trim().toLowerCase();
    if (configured === 'staging') return 'Staging';
    if (configured === 'production' || process.env.NODE_ENV === 'production') return 'Production';
    return configured ? configured[0].toUpperCase() + configured.slice(1) : 'Development';
}
...
function verifySession(cookieVal) {
    ...
    // Wix identity is mandatory now — old password-era sessions had no `m`
    // and must be treated as invalid so their holders get bounced to the
    // Wix sign-in flow rather than silently continuing under a stale shape.
...
function makeSessionCookie({ memberId, email, name }) {
    ...
    // it. SameSite=Lax so the top-level navigation back from Wix keeps the
    // cookie, while cross-site POSTs can't ride it.
...
function loginPage({ error } = {}) {
    const nextParam = ''; // reserved for future ?next= support
...
<body class="min-h-screen bg-gray-50 flex items-center justify-center p-4">
  <div class="w-full max-w-sm bg-white rounded-2xl shadow-sm p-6 space-y-4 border border-gray-200">
    <div>
      <div class="text-lg font-semibold text-gray-900">${ADMIN_PRODUCT_NAME}</div>
      <div class="text-xs text-gray-500 mt-1">${adminEnvironmentLabel()} · Sign in with your Talent Transformation account. We never see your password.</div>
    </div>
    ${error ? `<div class="text-sm text-red-600">${escapeHtml(error)}</div>` : ''}
    ${configured
        ? `<a href="/auth/login${nextParam}" class="block w-full text-center bg-teal-600 hover:bg-teal-700 text-white text-sm font-semibold rounded-lg py-2">Sign in with TT account</a>`
        : `<div class="text-sm text-red-600">Sign-in is not configured for this environment.</div>`}
  </div>
</body></html>`;
}

// Shown when the sign-in worked but the account lacks the Coach Studio Admin
// badge (or when Wix could not tell us either way). Separate from the login
// page because the fix is not "type it again" — it is "ask the operator to
// add me". Kept plain HTML rather than shellChrome() because the shell
// itself is behind the gate this page explains.
...
async function handle(req, res) {
    ...
    // Wix returns `code` and `state` after the '#', so the server never
    // sees them. This HTML runs a tiny script that reads the fragment and
    // POSTs it to /auth/wix/complete on the same origin.
    if (req.method === 'GET' && (url === '/auth/callback' || url.startsWith('/auth/callback?'))) {
        const html = `<!doctype html>
<html><head>
<meta charset="utf-8">
<title>${ADMIN_PRODUCT_NAME} — Signing in…</title>
<meta name="viewport" content="width=device-width,initial-scale=1">
<link rel="icon" type="image/svg+xml" href="${FAVICON_HREF}">
<script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="min-h-screen bg-gray-50 flex items-center justify-center p-4">
  <div class="w-full max-w-sm bg-white rounded-2xl shadow-sm p-6 space-y-4 border border-gray-200">
    <div class="text-lg font-semibold text-gray-900">Signing you in to ${ADMIN_PRODUCT_NAME}…</div>
  ...
  <script>
    (function () {
      ...
      // Cleared first, so the code is not left in the address bar to be
      // bookmarked, shared, or read over a shoulder.
      history.replaceState(null, '', location.pathname);
      var err = document.getElementById('err');
      if (!code || !state) {
        err.textContent = f.get('errorDescription') || f.get('error') ||
          'The sign-in provider did not send back a valid response to complete.';
    ...
    // Two failure modes: no cookie (bounce to sign-in), or cookie ok but
    // the Wix badge is missing / unreadable (denial page). The badge is
    // re-read per request (60s cache), so revoking access in Wix propagates
    // within a minute.
    if (!session) {
