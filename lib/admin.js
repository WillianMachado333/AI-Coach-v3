...
<script>
  (function () {
    // Mark active nav based on the current page title (data-title on <nav>).
    try {
      const nav = document.querySelector('.topo nav');
      const title = (nav && nav.dataset.title) || '';
      nav?.querySelectorAll('a[data-match]').forEach((a) => {
        if (title.startsWith(a.dataset.match)) a.classList.add('active');
      });
    } catch (_) {}
