...
/**
 * Identity providers Wix will run the whole flow for, ported from PIMS
 * (src/server/wix-oauth.ts WIX_IDP). With `idp` set on the redirect-session
 * request, Wix sends the person straight to the provider — no Wix dialog is
 * drawn at all. That matters for anyone who has never been on the site
 * before: Wix creates the member from the Google account directly, exactly
 * as picking Google inside the dialog would, instead of showing a Log In
 * screen for an account they don't have.
 *
 * Named here and resolved server-side on purpose: the browser sends
 * "google", never a provider id, so nobody can point sign-in at a provider
 * of their choosing.
 */
const WIX_IDP = {
...
/**
 * Returns { url, pkce } — send the person to url, and stash pkce in their
 * cookie. Pass idp: 'google' to skip the Wix dialog entirely and go
 * straight to that provider (see WIX_IDP above); omit it for the ordinary
 * Wix-hosted email/username dialog.
 */
async function beginLogin(idp) {
    const clientId = process.env.WIX_CLIENT_ID;
    if (!clientId) throw new Error('WIX_CLIENT_ID is not set on this deployment.');
    if (idp && !WIX_IDP[idp]) throw new Error(`Unknown identity provider: ${idp}`);
        ...
        body: JSON.stringify({
            auth: {
                authRequest: {
                    ...
                    state,
                    // Present only when a provider was asked for, so the
                    // ordinary flow reaches the Wix dialog unchanged.
                    ...(idp ? { idp: WIX_IDP[idp] } : {}),
                },
            },
        }),
