...
function callbackUri() {
    ...
    const configuredOrigin = process.env.AI_COACH_ADMIN_ORIGIN || process.env.PUBLIC_ORIGIN || '';
    if (configuredOrigin.trim()) {
        try {
            const candidate = /^https?:\/\//i.test(configuredOrigin.trim())
                ? configuredOrigin.trim()
                : `https://${configuredOrigin.trim()}`;
            const parsed = new URL(candidate);
            if ((parsed.protocol === 'https:' || parsed.protocol === 'http:') && !parsed.username && !parsed.password) {
                return `${parsed.origin}/auth/callback`;
            }
        } catch (_) {
            // Fall through to the Railway-provided origin rather than using an
            // invalid or ambiguous callback target.
        }
    }
