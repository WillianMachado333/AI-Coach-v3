...
function readMeta(courseId) {
    ...
    // Missing _meta.json: only synthesise a stub if the course dir itself
    // exists in either root. Otherwise return null so callers can render a
    // proper "not found" instead of a phantom edit form (which used to
    // silently create the record on Save).
    for (const root of [OVERLAY_ROOT, DEFAULTS_ROOT]) {
        try {
            if (fs.statSync(courseDir(root, courseId)).isDirectory()) {
