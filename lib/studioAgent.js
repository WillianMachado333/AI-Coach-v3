 ...
 *
 * Eric / Varsha chat with this agent to understand what Erica did in
...
// gpt-4.1-mini has vision + tool-calling and is cheap enough for the Studio.
// The env var overrides for people testing gpt-5 family.
const AGENT_MODEL = process.env.STUDIO_AGENT_MODEL || 'gpt-4.1-mini';
const MAX_TURNS = 8; // safety cap on tool-call iterations

const CHART_INSTRUCTIONS = [
    '',
    '=== RICH OUTPUT (charts and tables) — CHART-FIRST ===',
    'When comparing categories, distributions, ranges, or trends, append fenced blocks at the END of the prose. The UI renders them inline. Prefer drawing over describing.',
    '',
    'Trigger rules (lean toward YES):',
    '- ≥2 categories with counts → ```chart type=bar (yes, even for 2 categories — visual scan beats reading numbers).',
    '- ≥3 points over a time axis → ```chart type=line',
    '- ≥2 items × ≥2 attributes → ```table',
    '- single number → prose, no chart.',
    '',
    'NEVER ASK PERMISSION. Do not say "want me to draw a chart", "posso gerar um gráfico?", "quer que eu gere?", "should I plot this". If a chart would help, EMIT it. If it would not help, do not mention it.',
    'Do NOT list the numbers in prose then ask if you should visualise them — draw first, comment second.',
    'Do NOT re-read numbers verbatim in prose AFTER emitting a chart — the reader sees them.',
    '',
    'Chart block schema:',
    '```chart',
    '{"type":"bar"|"line","title":"Short title","xAxis":["label1","label2"],"series":[{"name":"Score","values":[10,20]}]}',
    '```',
    'At most 3 charts per answer, 4 series per chart, 60 points per series. Missing values should be omitted from the array, not sent as 0.',
    '',
    'Table block schema:',
    '```table',
    ...
    '',
    'Emit the prose first, then the fenced blocks.'
].join('\n');

const IMAGE_INSTRUCTIONS = [
    '',
    '=== ATTACHED IMAGES ===',
    'If a user turn includes attached images (screenshots, chart snapshots, whiteboard photos), TREAT THEM AS THE PRIMARY POINTER — the user usually pasted them because prose could not describe what they are asking about. Read what is visibly on the image (numbers, labels, red circles/arrows/highlights the user may have drawn) and answer with reference to those. If the image contains hand-drawn marks (red pen, arrows, circles), those marks are the user pointing — center the answer on what they marked, not on the rest of the image.',
    'When the user draws on a screenshot, the marks are more reliable than any description of them. Prefer citing what you see over what the user might have meant.'
].join('\n');
...
const SYSTEM_PROMPT = [
    ...
    'Your users are Eric (product owner) and Varsha (staff psychologist). They ask you to help them UNDERSTAND what Erica the coach did, not to change it.',
    '',
    'Your tools:',
    ...
    '- list_courses / read_course_artifact: pedagogical course content + competency framework + quiz index. Each course has three artifacts: "course-content" (lessons + quizzes), "competency-framework" (skills/behaviors/performance indicators), "quizzes-list" (short quiz index). Use these to ground answers about lessons, competencies, misconceptions, quiz questions.',
    '- list_activity_events: what the user has done on the platform (CleverTap).',
    '',
    'Voice + rules:',
    '- Speak to Eric and Varsha directly, executive tone. No engineering jargon in the answer body (never say "the SQL", "row", "NDJSON", "tester=exclude"); those live in the reasoning trace.',
    ...
    try {
        switch (name) {
            ...
            case 'list_courses': {
                ...
                const list = coursesStore.listCourses();
                return list.map((c) => {
...
function taskLabel(toolName, args) {
    switch (toolName) {
        ...
        case 'read_course_artifact':
            return 'Reading ' + (args?.course_id || 'course') + ' · ' + (args?.artifact || 'artifact');
        case 'list_activity_events':
