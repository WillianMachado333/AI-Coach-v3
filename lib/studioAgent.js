 ...
 */
async function runTurnStreamed({ history = [], userMessage, page = null }, onEvent) {
    if (!openaiClient) throw new Error('studioAgent: OpenAI client not initialised');
...
const FOLLOWUP_PROMPT = [
    ...
    'Rules:',
    '- First person from the analyst\'s voice ("Which sessions had…", "Why did…").',
    '- ≤70 characters each.',
    '- Prefer probing weakness or drilling into a specific claim from the last answer (small sample? mixed population? tool that failed?).',
