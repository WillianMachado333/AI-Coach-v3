...
const FRAMEWORKS_DIR = process.env.FRAMEWORKS_DIR
    || path.join(__dirname, '..', 'knowledge-base', 'frameworks');
        ...
        switch (name) {
            ...
            case 'list_frameworks': {
                try {
                    ...
                    return { error: 'frameworks dir not readable: ' + (e?.message || e) };
                }
            }
            case 'read_framework': {
                const name = String(args?.name || '').replace(/[^A-Za-z0-9_-]/g, '');
                if (!name) return { error: 'name required' };
                const p = path.join(FRAMEWORKS_DIR, name + '.md');
                if (!fs.existsSync(p)) return { error: 'framework not found: ' + name };
