...
/**
 * Merge new events into an existing timeline. CleverTap profile.json returns
 * ONE aggregate row per event name (count + first_seen + last_seen), so dedup
 * is by name — newer aggregate (higher ts) wins, and its `firstSeenTs` /
 * `count` are preserved by taking the max/max from either side. This fixes a
 * bug where the same event appeared twice in the timeline whenever its
 * last_seen ticked forward between fetches.
 */
function mergeAndTrim(existingEvents, newEvents) {
    ...
    for (const e of [...(existingEvents || []), ...(newEvents || [])]) {
        ...
        // Merge two aggregate rows for the same event name.
        byName.set(e.name, {
            name: e.name,
            ts: Math.max(prev.ts || 0, e.ts || 0),
            firstSeenTs: Math.min(
                prev.firstSeenTs || Number.POSITIVE_INFINITY,
                e.firstSeenTs || Number.POSITIVE_INFINITY
            ) || (prev.firstSeenTs || e.firstSeenTs || 0),
            count: Math.max(prev.count || 0, e.count || 0),
            props: e.props || prev.props || {},
            source: e.source || prev.source || 'cleverTap'
        });
    }
