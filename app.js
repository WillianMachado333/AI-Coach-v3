...
- Pauses: ${sanitize(effectivePauses)}`;
    ...
    async _syncPageContextIntoPrompt() {
        try {
            ...
            const pageBlock = [
                '',
                '=== CURRENT PAGE CONTEXT (live, situational — NOT your source of substance) ===',
                'This tells you WHERE the user is right now. It is a situational signal,',
                'nothing more. It is NOT your knowledge base.',
                '',
                'RULES (STRICT):',
                '- For any question about SUBSTANCE (what a trait means, how a competency',
                '  develops, why a result matters, what to do about it), call',
                '  search_knowledge FIRST. The semantic store — competency frameworks,',
                '  courses, quizzes, company information — is your source of depth.',
                '- Never source facts from the page. The page is a distillation shown TO',
                '  the user. Answering from it makes you fluent and shallow at the same',
                '  time — you would just be echoing what the user already sees.',
                '- You MAY cross-reference the page in phrasing: "what you are looking',
                '  at on the report shows X, and the reason that happens is [answer',
                '  grounded in search_knowledge]".',
                '- Reference the page naturally when the user says "this page", "this',
                '  report", "here" — but the DEPTH of your reply must come from the',
                '  store, not from the paragraph in front of them.',
                '',
                'Title: ' + (snap.title || '(untitled)'),
                'URL: ' + (snap.url || '(unknown)'),
                '',
                'Section outline (situational only):',
                headingsStr,
                '',
                'Visible text (situational only — do NOT quote as authoritative):',
                bodyText,
        ...
        try {
            ...
            if (data) {
                ...
                const knowledgeGroundingDirective = [
                    '',
                    '=== TONE — EMOJI DISCIPLINE ===',
                    'Weave 1–2 well-placed emojis into most turns. They are conversational punctuation, not decoration. WhatsApp / chat culture uses them constantly — text-only reads formal and distant. Rules:',
                    '- 1 emoji per turn is the default; 2 max on emotional-beat turns; 0 is fine on short factual replies.',
                    '- Place at the natural emotional beat of the sentence (opening, punchline, reflection close). Never trail-decorate the end of every sentence.',
                    '- Match the persona: Supportive/Erica leans warm (🌱 💛 🤔 ✨), Directive/Steve leans crisp (👉 ⚡ 🎯), Nurturing/Sarah leans tender (🫶 💫), Strengths/Sean leans affirming (💪 🔆). Never contradict the persona voice.',
                    '- Do NOT stack (avoid "🎉🎉🎉"). Do NOT use ambiguous or clinical emojis (💊 💉 🚨 🙏 combined with negation) that could read as directive medical/religious advice.',
                    '- Do NOT emoji a difficult moment ("you\'re describing burnout 🫠" is wrong). Silence beats a light emoji on heavy content.',
                    '- Voice mode: emojis are for text output. Do not speak the word "emoji" or describe them aloud.',
                    '',
                    ...
                    '',
                    'PAGE AWARENESS (get_page_context) — SITUATIONAL SIGNAL, NOT KNOWLEDGE:',
                    'You have a tool `get_page_context` that returns the title, URL,',
                    'headings and visible text of the page the user is currently on. This tells',
                    'you WHERE they are — it is not your source of substance.',
                    'Call it:',
                    '- At the start of a conversation on a new page so you can name what they',
                    '  are looking at when the user says "this" or "here".',
                    '- Whenever the user references "this page", "this report", "here", or',
                    '  asks about specific content on-screen.',
                    'THEN, before answering with substance about that content, call',
                    'search_knowledge against the semantic store (frameworks / courses /',
                    'quizzes / company). The page shows a summary; the store carries the',
                    'DEPTH the user does not see.',
                    'Do NOT read page text aloud. Do NOT source facts from the page.',
                    'You MAY cross-reference: "the paragraph you\'re on says X, and the reason',
                    'that matters is [answer grounded in search_knowledge]".',
                    '',
            ...
            if (functionName === 'search_web') {
                ...
                try {
                    ...
                    if (!snapshot) {
                        ...
                        result = JSON.stringify({
                            instruction: 'PAGE CONTEXT IS SITUATIONAL, NOT YOUR KNOWLEDGE BASE. Use this to know WHERE the user is. For any substantive answer about traits, competencies, results or what to do, call search_knowledge against the semantic store BEFORE responding. You may cross-reference the page ("what you see here says X, and the reason is [answer grounded in the store]") but never source facts from the page itself.',
                            url: snapshot.url,
